...
import {
  ...
  Search, Bell, MessageSquarePlus, CheckCircle2, XCircle, MinusCircle, FileText, ChevronRight, Moon, Sun, Upload, RotateCcw, ZoomIn, ShieldCheck, X, Pencil, PenLine, PaintBucket, ChevronDown, Info, History, BadgeCheck, PanelLeft,
} from 'lucide-react';
...
// R&D resume fixture must be the same layered MRP source used to verify the
// live Test & Certify screen. The old four-page mock contained only a single
// `GT PREVIEW` OCG, so the UI correctly—but misleadingly—showed one Preview
// chip instead of the press file's Bleed, Cut, and Safety controls.
import labelTemplatePdfUrl from '../../../../../../../attached_assets/20260814_GoodTunes_MRP_12-LBL100M-2_—_12in_Center_Labels_for_2_1788539107835.pdf';
...
async function extractGtLayers(doc: pdfjs.PDFDocumentProxy, pageNum: number): Promise<{ layers: GtLayer[]; layerNames: string[] }> {
  ...
  const countPathOps = (data: ArrayLike<number> | undefined, m: Matrix) => {
    ...
    const markTransformed = (X: number, Y: number) => {
    ...
    // A constructPath bbox can include cubic control handles that the curve
    // never reaches. Evaluate the Bezier extrema so circular OCG dimensions
    // come from the painted path rather than Illustrator's off-curve handles.
    const markCubic = (p0: [number, number], p1: [number, number], p2: [number, number], p3: [number, number]) => {
      const points = [p0, p1, p2, p3].map(([x, y]) => applyM(m, x, y));
      const at = (axis: 0 | 1, t: number) => {
        const mt = 1 - t;
        return mt ** 3 * points[0][axis] + 3 * mt ** 2 * t * points[1][axis] + 3 * mt * t ** 2 * points[2][axis] + t ** 3 * points[3][axis];
      };
      const roots = new Set<number>([0, 1]);
      for (const axis of [0, 1] as const) {
        const a = -points[0][axis] + 3 * points[1][axis] - 3 * points[2][axis] + points[3][axis];
        const b = 2 * (points[0][axis] - 2 * points[1][axis] + points[2][axis]);
        const c = points[1][axis] - points[0][axis];
        if (Math.abs(a) < 1e-9) {
          if (Math.abs(b) > 1e-9) roots.add(-c / b);
        } else {
      ...
      for (const t of roots) {
        if (t >= 0 && t <= 1) markTransformed(at(0, t), at(1, t));
      }
    };
    if (data && data.length) {
  ...
  for (let i = 0; i < ol.fnArray.length; i++) {
    ...
    if (fn === OPS.save) ctmStack.push(ctm);
      ...
      if (layerName) usedLayerNames.add(layerName);
      ...
      const paintedBoxes = pc.subs.length ? pc.subs : [{
        minX: applyM(ctm, mm3[0], mm3[1])[0],
        minY: applyM(ctm, mm3[0], mm3[1])[1],
        maxX: applyM(ctm, mm3[2], mm3[3])[0],
        maxY: applyM(ctm, mm3[2], mm3[3])[1],
      }];
      ...
      for (const box of paintedBoxes) {
        b.minX = Math.min(b.minX, box.minX); b.maxX = Math.max(b.maxX, box.maxX);
        b.minY = Math.min(b.minY, box.minY); b.maxY = Math.max(b.maxY, box.maxY);
      }
  ...
  // Illustrator/PDF can leave disabled, orphaned OCG entries in the document
  // catalog after those layers are removed from every page. Only report layer
  // names that a page content stream actually references.
  return { layers, layerNames: [...usedLayerNames] };
}
...
const objectTextDecoder = new TextDecoder('latin1');
...
function readPdfObjectDictionary(bytes: Uint8Array, ref: number) {
  ...
  const endObjectAt = findAscii(bytes, 'endobj', start);
  const candidates = [streamAt, endObjectAt].filter((value) => value >= start);
  const end = candidates.length ? Math.min(...candidates) : Math.min(start + 65536, bytes.length);
  return objectTextDecoder.decode(bytes.subarray(start, Math.min(end, start + 65536)));
}

function imageColorKind(bytes: Uint8Array, ref: number, cache: Map<number, PdfColorKind>, depth = 0): PdfColorKind {
  const cached = cache.get(ref);
  if (cached) return cached;
  if (depth > 4) return 'unknown';
  const dictionary = readPdfObjectDictionary(bytes, ref);
  if (!dictionary) return 'unknown';
  let kind: PdfColorKind = 'unknown';
  if (/\/(?:Separation|DeviceN)\b/.test(dictionary)) kind = 'spot';
  else if (/\/N\s*4\b/.test(dictionary) || /\/DeviceCMYK\b/.test(dictionary)) kind = 'cmyk';
  else if (/\/N\s*3\b/.test(dictionary) || /\/DeviceRGB\b/.test(dictionary)) kind = 'rgb';
  else if (/\/N\s*1\b/.test(dictionary) || /\/DeviceGray\b/.test(dictionary)) kind = 'gray';
  else {
    const indirect = dictionary.match(/\/(?:ColorSpace|ICCBased)\s*\[?\s*(\d+)\s+\d+\s+R/);
    if (indirect) kind = imageColorKind(bytes, Number(indirect[1]), cache, depth + 1);
  }
  cache.set(ref, kind);
  return kind;
}

function buildImageColorIndex(bytes: Uint8Array) {
  ...
  for (const marker of ['/Subtype/Image', '/Subtype /Image']) {
    ...
    while (cursor < bytes.length) {
      ...
      if (hit < 0) break;
      const prefixStart = Math.max(0, hit - 8192);
      const prefix = objectTextDecoder.decode(bytes.subarray(prefixStart, hit));
      const matches = [...prefix.matchAll(/(\d+)\s+\d+\s+obj/g)];
      const ref = Number(matches.at(-1)?.[1]);
      if (Number.isFinite(ref)) refs.add(ref);
      cursor = hit + marker.length;
    }
  }
  const objectCache = new Map<number, PdfColorKind>();
  const index = new Map<string, PdfColorKind[]>();
  const dictionaries = new Map<number, string>();
  const softMaskRefs = new Set<number>();
  for (const ref of refs) {
    const dictionary = readPdfObjectDictionary(bytes, ref);
    dictionaries.set(ref, dictionary);
    const softMask = Number(dictionary.match(/\/SMask\s*(\d+)\s+\d+\s+R/)?.[1]);
    if (Number.isFinite(softMask)) softMaskRefs.add(softMask);
  }
  for (const ref of refs) {
    if (softMaskRefs.has(ref)) continue;
    const dictionary = dictionaries.get(ref) ?? '';
    ...
    const kind = imageColorKind(bytes, ref, objectCache);
...
async function analyzePdfArtPage(
  doc: pdfjs.PDFDocumentProxy,
  pageNum: number,
  imageColorIndex: Map<string, PdfColorKind[]>,
  ...
  const stateStack: Array<{ ctm: Matrix; fillColor: PdfColorKind; strokeColor: PdfColorKind }> = [];
  const ppis: number[] = [];
  let rasterImageCount = 0;
  let unresolvedRasterImages = 0;
  let hasCmyk = false;
  let hasRgb = false;
  let hasGray = false;
  let hasSpot = false;

  const applyColorKind = (kind: PdfColorKind) => {
    if (kind === 'cmyk') hasCmyk = true;
    else if (kind === 'rgb') hasRgb = true;
    else if (kind === 'gray') hasGray = true;
    else if (kind === 'spot') hasSpot = true;
    else unresolvedRasterImages += 1;
  };
  ...
  const recordImage = (pixelWidth: number, pixelHeight: number, colors: PdfColorKind[]) => {
    ...
    if (colors.length === 0) applyColorKind('unknown');
    else colors.forEach(applyColorKind);
  };
  ...
  const textPaintOps = new Set([OPS.showText, OPS.showSpacedText, OPS.nextLineShowText, OPS.nextLineSetSpacingShowText]);
  const colorMaskOps = new Set([OPS.paintImageMaskXObject, OPS.paintImageMaskXObjectRepeat, OPS.paintImageMaskXObjectGroup, OPS.paintSolidColorImageMask]);

  for (let index = 0; index < operators.fnArray.length; index++) {
    const fn = operators.fnArray[index];
    const args = operators.argsArray[index] as unknown[];
    if (fn === OPS.save) stateStack.push({ ctm: [...ctm] as Matrix, fillColor, strokeColor });
    ...
    else if (fillOps.has(fn)) applyColorKind(fillColor);
    else if (strokeOps.has(fn)) applyColorKind(strokeColor);
    else if (fillStrokeOps.has(fn)) {
      applyColorKind(fillColor);
      applyColorKind(strokeColor);
    } else if (textPaintOps.has(fn) || colorMaskOps.has(fn)) applyColorKind(fillColor);
    else if (fn === OPS.paintImageXObject) {
      const pixelWidth = Number(args[1] ?? 0);
      const pixelHeight = Number(args[2] ?? 0);
      recordImage(pixelWidth, pixelHeight, imageColorIndex.get(`${pixelWidth}x${pixelHeight}`) ?? []);
    } else if (fn === OPS.paintInlineImageXObject) {
      const image = args[0] as PdfImageObject | undefined;
      const pixelWidth = Number(image?.width ?? 0);
      const pixelHeight = Number(image?.height ?? 0);
      recordImage(pixelWidth, pixelHeight, imageColorIndex.get(`${pixelWidth}x${pixelHeight}`) ?? []);
    } else if (fn === OPS.paintImageXObjectRepeat || fn === OPS.paintInlineImageXObjectGroup) {
      rasterImageCount += 1;
      unresolvedRasterImages += 1;
    }
  }
...
const ZONE_ORDER = ['Bleed', 'Cut', 'Safety', 'Spine', 'Front Cover', 'Back Cover', 'Front Safety', 'Back Safety', 'Artboard'];
const ZONE_COLORS: Record<string, string> = {
  Bleed: '#e0245e', Cut: '#319ED8', Spine: '#b07ce8', 'Front Cover': '#f5a623', 'Back Cover': '#f5a623',
  Safety: '#34c98e', 'Front Safety': '#34c98e', 'Back Safety': '#34c98e', Artboard: '#98989d',
};
...
function resolutionCheck(analysis: PdfPageAnalysis | null): CheckRow {
  if (!analysis) return { param: 'Resolution', tone: 'na', detail: 'Source resolution needs a PDF.' };
  if (analysis.rasterImageCount === 0) {
    return { param: 'Resolution', tone: 'pass', detail: 'Vector artwork — no raster resolution floor applies' };
  }
...
function colorAndResolutionCheck(analysis: PdfPageAnalysis | null): CheckRow {
  const resolution = resolutionCheck(analysis);
  if (!analysis) return { param: 'Color & resolution', tone: 'na', detail: 'Ink and image checks need a PDF.' };
  const modes = [
    analysis.hasCmyk ? 'CMYK' : null,
    analysis.hasSpot ? 'spot' : null,
    analysis.hasGray ? 'grayscale' : null,
  ].filter(Boolean);
  const mode = modes.join(' + ');
  if (analysis.hasRgb) {
    return { param: 'Color & resolution', tone: 'fail', detail: 'RGB content detected — convert all artwork to CMYK or approved spot inks' };
  }
  if (analysis.unresolvedRasterImages > 0 || modes.length === 0) {
    return { param: 'Color & resolution', tone: 'na', detail: 'Could not verify every source color space in this page.' };
  }
  if (resolution.tone === 'fail') {
    return { param: 'Color & resolution', tone: 'fail', detail: `${mode} ink is present, but raster resolution is below the ${MIN_PRINT_PPI} PPI floor` };
  }
  if (resolution.tone === 'na') {
    return { param: 'Color & resolution', tone: 'na', detail: `${mode} ink is present; raster resolution still needs verification` };
  }
  return {
    param: 'Color & resolution',
    tone: 'pass',
    detail: `${mode} — ${analysis.hasSpot ? 'spot preserved, ' : ''}ink and image resolution both clear`,
  };
}

type PageViewState = {
  zoom: number;
  panC: { x: number; y: number } | null;
  activeZones: Set<string>;
};

export default function PressTemplateLiveTestRndOwned() {
  ...
  const [pageRailOpen, setPageRailOpen] = useState(() => typeof window !== 'undefined' && window.innerWidth >= 768);
  const pageViews = useRef<Record<number, PageViewState>>({});
  ...
  const [templateOpacity, setTemplateOpacity] = useState(0.55);
  ...
  const [templatePanelOpen, setTemplatePanelOpen] = useState(false); // template-opacity dropdown on the Template chip
  ...
  const loadTemplate = async (f: File, displayName?: string) => {
    ...
    try {
      ...
      for (let pageNumber = 1; pageNumber <= doc.numPages; pageNumber++) {
        const [{ img, wMm, hMm }, { layers, layerNames }] = [await renderPage(doc, pageNumber), await extractGtLayers(doc, pageNumber)];
        ...
        pages.push({ name: displayName ?? f.name, img, wMm, hMm, layers: gt, layerNames, pageCount: doc.numPages });
      }
  ...
  const resumeDraft = async () => {
    ...
    try {
      ...
      await loadTemplate(
        new File(
          [blob],
          '20260814_GoodTunes_MRP_12-LBL100M-2 — 12in Center Labels for 2LP — R072326.pdf',
  ...
  const discardDraft = () => { setResumeOffer(false); window.location.hash = '#/PressTemplatesIndexRnd'; };
  ...
  const onPickArt = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (f.type === 'application/pdf' || f.name.toLowerCase().endsWith('.pdf')) {
        ...
        // pdf.js may transfer its Uint8Array to the worker. Keep our source copy
        // available for image color-space dictionaries.
        const doc = await pdfjs.getDocument({ data: sourceBytes.slice() }).promise;
        const pages: ArtState[] = [];
        const imageColorIndex = buildImageColorIndex(sourceBytes);
        for (let pageNumber = 1; pageNumber <= doc.numPages; pageNumber++) {
          const { img, wMm, hMm } = await renderPage(doc, pageNumber);
          const { layerNames } = await extractGtLayers(doc, pageNumber);
          const gtLayerNames = layerNames.filter((n) => n.trim().toUpperCase().startsWith('GT'));
          const analysis = await analyzePdfArtPage(doc, pageNumber, imageColorIndex);
          pages.push({ name: f.name, img, wMm, hMm, pageCount: doc.numPages, gtLayerNames, analysis });
        }
        setArtPages(pages);
        setArt(pages[currentPage] ?? null);
        setPageCountError(doc.numPages === templatePages.length
          ? null
          : `Page count mismatch: this template has ${templatePages.length} ${templatePages.length === 1 ? 'page' : 'pages'}, but “${f.name}” has ${doc.numPages}. Upload art with exactly ${templatePages.length} ${templatePages.length === 1 ? 'page' : 'pages'}; pages are paired only in order.`);
        ...
        const raster = { name: f.name, img, wMm: null, hMm: null, pageCount: 1, gtLayerNames: [], analysis: null };
        setArtPages([raster]);
        setArt(currentPage === 0 ? raster : null);
        setPageCountError(templatePages.length === 1
          ? null
          : `Page count mismatch: this template has ${templatePages.length} pages, but “${f.name}” has 1. Upload art with exactly ${templatePages.length} pages; pages are paired only in order.`);
        setShowTemplate(false);
      }
    } catch (err) {
      setArt(null);
  ...
  const checks: CheckRow[] = useMemo(() => {
    ...
    rows.push(resolutionCheck(art.analysis));
    rows.push(colorAndResolutionCheck(art.analysis));
    ...
    return rows;
  }, [template, art, bleedBox, cutBox, currentPage, pageCountError]);

  const measured = checks.filter((c) => c.tone !== 'na');
  const pageStates = useMemo(() => templatePages.map((templatePage, index): 'untested' | 'fail' | 'pass' => {
    const artPage = artPages[index];
    if (!artPage || pageCountError) return artPage ? 'fail' : 'untested';
    const bleedLayer = templatePage.layers.find((layer) => layer.zone === 'Bleed' && layer.kind === 'line')
      ?? templatePage.layers.find((layer) => layer.zone === 'Bleed');
    if (artPage.wMm === null || artPage.hMm === null) return 'untested';
    if (artPage.gtLayerNames.length > 0) return 'fail';
    if (bleedLayer) {
      const covers = (w: number, h: number) => artPage.wMm! >= w - 1 && artPage.hMm! >= h - 1;
      const sizePass = covers(bleedLayer.wMm, bleedLayer.hMm) || covers(bleedLayer.hMm, bleedLayer.wMm);
      if (!sizePass) return 'fail';
    }
    const resolution = resolutionCheck(artPage.analysis);
    const color = colorAndResolutionCheck(artPage.analysis);
    if (resolution.tone === 'fail' || color.tone === 'fail') return 'fail';
    if (resolution.tone === 'na' || color.tone === 'na') return 'untested';
    return 'pass';
  }), [templatePages, artPages, pageCountError]);
  const allPass = templatePages.length > 0 && pageStates.length === templatePages.length && pageStates.every((state) => state === 'pass');
  ...
  const currentPageFailed = measured.some((check) => check.tone === 'fail');
  const anyPageFailed = pageStates.some((state) => state === 'fail');
  const verdictIsFail = currentPageFailed || anyPageFailed;
  const verdictWord = allPass ? 'Pass' : verdictIsFail ? 'Flagged' : 'Visual only';
  const verdictCount = templatePages.length > 1
    ? `${pageStates.filter((state) => state === 'pass').length} of ${templatePages.length} pages passed`
    : measured.length > 0
      ? `${measured.filter((check) => check.tone === 'pass').length} of ${measured.length} passed`
      : '';
  const selectPage = (index: number) => {
    if (index === currentPage || !templatePages[index]) return;
    pageViews.current[currentPage] = { zoom, panC, activeZones: new Set(activeZones) };
    const nextView = pageViews.current[index];
    setCurrentPage(index);
    setTemplate(templatePages[index]);
  ...
  const saveAndExit = () => {
    ...
    window.location.hash = '#/PressTemplatesIndexRnd';
  };
      ...
      <div className="mx-auto w-full" style={{ maxWidth: 1240, padding: '32px 40px 96px' }}>
        <nav aria-label="breadcrumb" data-testid="breadcrumb-livetest">
          <ol className="flex flex-wrap items-center gap-2 text-[13px]" style={{ color: t.faint }}>
            <li className="inline-flex items-center"><button type="button" onClick={() => { window.location.hash = '#/PressTemplatesIndexRnd'; }} className={cn('transition-colors', t.hoverInk)} data-testid="link-breadcrumb-templates">Templates</button></li>
        ...
        {template && (
          <div className="mt-6">
            ...
            {art && (
              <div className="mb-4" key={art.name}>
                ...
                >
                  ...
                  >
                    <span className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: allPass ? t.readyWash : verdictIsFail ? t.critWash : t.neutralWash }}>
                      {allPass
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-[13.5px] font-semibold" style={{ color: t.ink }} data-testid="text-verdict">
                        {allPass ? 'Pass! All measured checks passed' : currentPageFailed ? 'Fail! Measured checks flag issues' : anyPageFailed ? 'Flagged! Another page needs attention' : 'Visual only — nothing to measure'}
            ...
            {pageCountError && templatePages.length === 1 && (
              <div className="mb-4 rounded-2xl px-5 py-3 text-[12.5px] font-medium" style={{ color: t.crit, backgroundColor: t.critWash, border: `1px solid ${t.crit}` }} role="alert" data-testid="error-page-count">
                {pageCountError}
              </div>
            )}
                  ...
                  {busy === 'art' ? (
                    ...
                    <>
                      ...
                      {dirty && (
                        <button
                          type="button"
                          onClick={() => { window.location.hash = '#/PressTemplatesIndexRnd'; }}
                          disabled={busy !== null}
                      ...
                      <button
                        type="button"
                        onClick={() => { if (dirty) saveAndExit(); else window.location.hash = '#/PressTemplatesIndexRnd'; }}
                        disabled={busy !== null}
              ...
              <div className="flex flex-wrap items-center gap-2 px-6 py-3" style={{ borderBottom: `1px solid ${t.hairline}` }}>
                {templatePages.length > 0 && (
                  <button
                    ...
                    aria-controls="template-page-rail"
                    aria-label={pageRailOpen ? 'Hide page thumbnails' : 'Show page thumbnails'}
                    title={pageRailOpen ? 'Hide page thumbnails' : 'Show page thumbnails'}
                    className={cn('inline-flex h-8 w-8 items-center justify-center rounded-full transition-colors', t.hoverWash)}
                    style={{ color: pageRailOpen ? t.ink : t.subink, backgroundColor: pageRailOpen ? t.soft : 'transparent' }}
                    data-testid="button-toggle-page-rail"
                  >
                    <PanelLeft style={{ width: 15, height: 15 }} aria-hidden="true" />
                  </button>
                )}
                ...
                {showLayers && (
                  <>
                    ...
                    <div
                      ...
                      style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}`, width: 420, maxWidth: 'calc(100vw - 32px)', top: 'calc(100% + 8px)', right: 0 }}
                      role="dialog"
                      aria-label="Layers read from the file"
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between gap-3 mb-3">
                  <div className="flex flex-wrap items-center gap-2">
                    {/* Template display — off by default during a test; template
                        opacity lives in its dropdown, not the header. */}
                    {art && (
                      <div className="relative flex-shrink-0">
                        <span
                ...
                {pageCountError && templatePages.length > 1 && (
                  <div className="mb-3 rounded-xl px-3 py-2 text-[12px] font-medium" style={{ color: t.crit, backgroundColor: t.critWash }} role="alert" data-testid="error-page-count">
                    {pageCountError}
                  </div>
                )}
                <div className="relative flex items-stretch justify-center gap-3">
                {templatePages.length > 0 && pageRailOpen && (
                  <aside
                    id="template-page-rail"
                    className="absolute left-0 top-0 bottom-0 z-20 w-28 overflow-y-auto rounded-xl p-2 md:static md:z-auto md:flex-shrink-0"
                    style={{ backgroundColor: t.soft, border: `1px solid ${t.hairline}` }}
                    aria-label="Template pages"
                    data-testid="template-page-navigator"
                  >
                    <div className="mb-2 px-1">
                      <div className="text-[11px] font-semibold" style={{ color: t.ink }}>Page {currentPage + 1} of {templatePages.length}</div>
                      <div className="mt-0.5 text-[9.5px] leading-tight" style={{ color: allPass ? t.ready : t.subink }}>{aggregateExplanation}</div>
                    </div>
                    <div className="flex flex-col gap-2" role="tablist" aria-orientation="vertical">
                      {templatePages.map((page, index) => {
                        ...
                        const stateColor = state === 'pass' ? t.ready : state === 'fail' ? t.crit : t.faint;
                        const artPage = artPages[index];
                        const pageBleed = page.layers.find((layer) => layer.zone === 'Bleed' && layer.kind === 'line')
                          ?? page.layers.find((layer) => layer.zone === 'Bleed');
                        const pageCut = page.layers.find((layer) => layer.zone === 'Cut' && layer.kind === 'line')
                          ?? page.layers.find((layer) => layer.zone === 'Cut');
                        const pageAnchor = pageBleed ?? pageCut ?? { xMm: 0, yMm: 0, wMm: page.wMm, hMm: page.hMm };
                        const pageArtRect = artPage
                          ? artPage.wMm === null || artPage.hMm === null
                            ? pageAnchor
                            : {
                                xMm: pageAnchor.xMm + pageAnchor.wMm / 2 - artPage.wMm / 2,
                                yMm: pageAnchor.yMm + pageAnchor.hMm / 2 - artPage.hMm / 2,
                                wMm: artPage.wMm,
                                hMm: artPage.hMm,
                              }
                          : null;
                        return (
                          <button
                            ...
                            aria-label={`Page ${index + 1}, ${state}`}
                            onClick={() => selectPage(index)}
                            className="w-full rounded-lg p-1.5 text-left transition-colors focus:outline-none focus:ring-2"
                            style={{
                              border: `2px solid ${selected ? t.blue : 'transparent'}`,
                              backgroundColor: selected ? t.card : 'transparent',
                            }}
                            data-testid={`button-template-page-${index + 1}`}
                          >
                            <div className="grid h-20 w-full place-items-center overflow-hidden rounded bg-white">
                              <div
                                className="relative max-h-full max-w-full overflow-hidden"
                                style={{
                                  aspectRatio: `${page.wMm} / ${page.hMm}`,
                                  height: page.hMm >= page.wMm ? '100%' : undefined,
                                  width: page.hMm < page.wMm ? '100%' : undefined,
                                }}
                                data-testid={`thumbnail-composite-page-${index + 1}`}
                              >
                                {artPage && pageArtRect && (
                                  <img
                                    src={artPage.img}
                                    ...
                                    className="absolute"
                                    style={{
                                      left: pct(pageArtRect.xMm, page.wMm),
                                      top: pct(pageArtRect.yMm, page.hMm),
                                      width: pct(pageArtRect.wMm, page.wMm),
                                      height: pct(pageArtRect.hMm, page.hMm),
                                    }}
                                    draggable={false}
                                ...
                                {(!artPage || showTemplate) && (
                                  <img
                                    ...
                                    style={{
                                      opacity: artPage ? templateOpacity : 1,
                                      mixBlendMode: artPage ? 'multiply' : 'normal',
                                    }}
                                    draggable={false}
                            ...
                            <span className="mt-1 flex items-center justify-between gap-1 text-[10px] font-medium" style={{ color: selected ? t.ink : t.subink }}>
                              <span>Page {index + 1}</span>
                              <span className="h-2 w-2 rounded-full" style={{ backgroundColor: stateColor }} aria-hidden />
                            </span>
                          </button>
                        );
                      })}
                 ...
                 >
                  ...
                  {(!art || showTemplate) && (
                    <img
                      ...
                      style={{
                        opacity: art ? templateOpacity : 1,
                        mixBlendMode: art ? 'multiply' : 'normal',
                      }}
                      draggable={false}
                      data-testid="img-template-overlay"
                    />
                  )}
                      ...
                      <div key={zone}>
                        ...
                        <div
                          ...
                          style={{
                            ...
                            border: `1.5px ${zone === 'Bleed' ? 'dashed' : zone.includes('Safety') ? 'dotted' : 'solid'} ${c}`,
