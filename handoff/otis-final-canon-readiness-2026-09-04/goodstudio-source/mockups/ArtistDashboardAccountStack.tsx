...
import { PackageArtworkDialog as CanonPackageArtworkDialog, VinylPackageArtwork } from './VinylPackageArtwork';
import { Interactive as OtisTracksInteractive } from './OtisTracksInteractive';
import { GoodDeedArtistPreview } from './GoodDeedArtistPreview';
import goodDeedOtisSquare from '../assets/gooddeed-otis-production-2026-09/renders/square-1080x1080.png';
import goodDeedOtisPortrait from '../assets/gooddeed-otis-production-2026-09/renders/portrait-1080x1350.png';
import goodDeedOtisStory from '../assets/gooddeed-otis-production-2026-09/renders/story-1080x1920.png';
import {
  Award,
  Banknote,
  BadgeCheck,
...
const goodDeedPrintPdfs = {
  letter: new URL('../assets/gooddeed-californialand-letter.pdf', import.meta.url).href,
  a4: new URL('../assets/gooddeed-californialand-a4.pdf', import.meta.url).href,
};

const goodDeedPrintPreviews = {
  letter: new URL('../assets/gooddeed-californialand-letter.png', import.meta.url).href,
  a4: new URL('../assets/gooddeed-californialand-a4.png', import.meta.url).href,
};
    ...
    <div className="flex flex-col gap-5" data-testid="artist-dashboard-body">
      ...
      <div className={cn('flex gap-3 flex-wrap', isSuperAdmin ? 'items-center justify-end' : 'items-end justify-between')}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => onAction('Orders link held in comparison frame.')}
            className={cn('inline-flex h-9 items-center gap-2 rounded-full px-4 text-[14px] font-medium transition-colors', t.hoverWash)}
...
// ─── Artist destination bodies ────────────────────────────────────────────
// These are deliberately data-forward pages rather than navigation stubs. The
// fixtures mirror the dashboard's conventions so the R&D route is useful as a
// truthful thumbnail and as a deep-linked review surface.
const MOCK_AUDIENCE_SEGMENTS = [
  { id: 'returning', label: 'Returning listeners', value: '12,408', detail: '+18.4% vs prior period', share: 68 },
  { id: 'new', label: 'New listeners', value: '5,930', detail: '+9.7% vs prior period', share: 32 },
  { id: 'buyers', label: 'Collectors who bought', value: '224', detail: '1.2% of active listeners', share: 12 },
];
const MOCK_AUDIENCE_SOURCES = [
  { label: 'GoodTunes profile', value: '9,860', share: 54 },
  { label: 'Direct links', value: '4,420', share: 24 },
  { label: 'Spotify', value: '2,760', share: 15 },
  { label: 'Social referrals', value: '1,380', share: 7 },
];

function AudienceBody({ t, onAction }: { t: Theme; onAction: (msg: string) => void }) {
  const [segment, setSegment] = useState<'listeners' | 'buyers'>('listeners');
  const listenerValues = [28, 34, 31, 42, 45, 48, 54, 52, 61, 67, 72, 78];
  const buyerValues = [12, 15, 13, 20, 19, 23, 22, 27, 25, 31, 35, 38];
  const values = segment === 'listeners' ? listenerValues : buyerValues;
  const points = values.map((v, i) => `${24 + i * 48},${205 - v * 2.15}`).join(' ');
  return (
    <div className="flex flex-col gap-5" data-testid="artist-audience-body">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div><h1 className="text-[30px] font-semibold" style={{ color: t.ink, letterSpacing: '-.025em' }}>Audience.</h1><p className="mt-1 text-[14px]" style={{ color: t.subink }}>Understand who is listening, buying, and coming back.</p></div>
        <RangeSwitcher value="30d" onChange={() => {}} t={t} />
      </div>
      <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))' }}>
        {MOCK_AUDIENCE_SEGMENTS.map((item) => (
          <div key={item.id} className="rounded-2xl p-5" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }} data-testid={`audience-metric-${item.id}`}>
            <div className="text-[13px] font-medium" style={{ color: t.subink }}>{item.label}</div>
            <div className="mt-3 text-[32px] tabular-nums font-semibold" style={{ color: t.ink, letterSpacing: '-.03em' }}>{item.value}</div>
            <div className="mt-2 text-[12px]" style={{ color: item.id === 'buyers' ? t.subink : t.ready }}>{item.detail}</div>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <div className="xl:col-span-2 rounded-2xl p-6" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }}>
          <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
            <h2 className="text-[20px]"><span className="font-semibold" style={{ color: t.ink }}>Growth.</span>{' '}<span className="font-medium" style={{ color: t.subink }}>Your audience over time.</span></h2>
            <div className="inline-flex items-center p-1 rounded-full" style={{ backgroundColor: t.cardSoft }}>
              {(['listeners', 'buyers'] as const).map((item) => <button key={item} type="button" onClick={() => setSegment(item)} className="px-3 h-7 rounded-full text-[12px] capitalize" style={{ color: segment === item ? t.ink : t.subink, backgroundColor: segment === item ? t.card : undefined, boxShadow: segment === item ? t.pillShadow : undefined, fontWeight: segment === item ? 600 : 500 }} data-testid={`button-audience-${item}`}>{item}</button>)}
            </div>
          </div>
          <div className="min-h-[260px]">
            <svg viewBox="0 0 580 235" className="w-full h-full" preserveAspectRatio="none" aria-label={`${segment} growth`}>
              {[35, 80, 125, 170, 215].map((y) => <line key={y} x1="24" x2="565" y1={y} y2={y} stroke={t.hairline} strokeDasharray="3 3" />)}
              <polyline points={points} fill="none" stroke={BLUE} strokeWidth="2.5" vectorEffect="non-scaling-stroke" />
              {['Aug 1', 'Aug 7', 'Aug 14', 'Aug 21', 'Aug 30'].map((x, i) => <text key={x} x={24 + i * 133} y="232" fontSize="10" fill={t.faint}>{x}</text>)}
            </svg>
          </div>
        </div>
        <div className="rounded-2xl p-6" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }}>
          <h2 className="text-[20px]"><span className="font-semibold" style={{ color: t.ink }}>Sources.</span>{' '}<span className="font-medium" style={{ color: t.subink }}>Where people find you.</span></h2>
          <div className="mt-5 space-y-4">
            {MOCK_AUDIENCE_SOURCES.map((source) => <button key={source.label} type="button" onClick={() => onAction(`${source.label} audience detail opened.`)} className={cn('w-full text-left', t.hoverWash)} data-testid={`button-audience-source-${source.label.toLowerCase().replace(/\s/g, '-')}`}>
              <div className="flex justify-between text-[13px]"><span style={{ color: t.ink }}>{source.label}</span><span className="font-semibold tabular-nums" style={{ color: t.ink }}>{source.value}</span></div>
              <div className="mt-2 h-1.5 rounded-full" style={{ backgroundColor: t.cardSoft }}><div className="h-full rounded-full" style={{ width: `${source.share}%`, backgroundColor: BLUE }} /></div>
            </button>)}
          </div>
        </div>
      </div>
      <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }}>
        <div className="px-6 py-5 flex items-center justify-between"><h2 className="text-[20px]"><span className="font-semibold" style={{ color: t.ink }}>Places.</span>{' '}<span className="font-medium" style={{ color: t.subink }}>Where your listeners are.</span></h2><button type="button" onClick={() => onAction('Audience export requested.')} className="text-[13px] font-medium" style={{ color: t.blue }} data-testid="button-export-audience">Export audience</button></div>
        <div className="grid grid-cols-2 md:grid-cols-4" style={{ borderTop: `1px solid ${t.hairline}` }}>
          {[['United States', '8,420'], ['Canada', '2,180'], ['United Kingdom', '1,360'], ['Australia', '920']].map(([place, count]) => <div key={place} className="px-6 py-4" style={{ borderRight: `1px solid ${t.hairline}` }}><div className="text-[13px]" style={{ color: t.subink }}>{place}</div><div className="mt-1 text-[20px] font-semibold tabular-nums" style={{ color: t.ink }}>{count}</div></div>)}
        </div>
      </div>
    </div>
  );
...
const MOCK_ORDERS = [
  { id: 'GT-10482', buyer: 'Maya Chen', item: 'CALIFORNIALAND · 2 vinyl', total: '$58', status: 'Paid', date: 'Today, 9:42 AM' },
  { id: 'GT-10476', buyer: 'Jon Bell', item: 'CALIFORNIALAND · 1 vinyl', total: '$34', status: 'Packing', date: 'Yesterday' },
  { id: 'GT-10451', buyer: 'Riley Hart', item: 'CALIFORNIALAND · Digital', total: '$12', status: 'Fulfilled', date: 'Aug 28' },
  { id: 'GT-10408', buyer: 'Sam Ortiz', item: 'CALIFORNIALAND · 1 vinyl', total: '$34', status: 'Fulfilled', date: 'Aug 24' },
];

function OrdersBody({ t, onAction }: { t: Theme; onAction: (msg: string) => void }) {
  const [filter, setFilter] = useState<'all' | 'open' | 'fulfilled'>('all');
  const visible = MOCK_ORDERS.filter((order) => filter === 'all' || (filter === 'open' ? order.status !== 'Fulfilled' : order.status === 'Fulfilled'));
  return (
    <div className="flex flex-col gap-5" data-testid="artist-orders-body">
      <div className="flex items-end justify-between gap-4 flex-wrap"><div><h1 className="text-[30px] font-semibold" style={{ color: t.ink, letterSpacing: '-.025em' }}>Orders.</h1><p className="mt-1 text-[14px]" style={{ color: t.subink }}>Keep CALIFORNIALAND orders moving from paid to delivered.</p></div><button type="button" onClick={() => onAction('Order export requested.')} className="inline-flex items-center gap-2 h-9 px-4 rounded-full text-[13px] font-medium" style={{ border: `1px solid ${t.hairline}`, color: t.ink }} data-testid="button-export-orders"><Download className="w-3.5 h-3.5" />Export orders</button></div>
      <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
        {[['Open orders', '2', 'Need attention'], ['Gross sales', '$486', 'last 30 days'], ['Units shipped', '186', 'all releases']].map(([label, value, note]) => <div key={label} className="rounded-2xl p-5" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }}><div className="text-[13px]" style={{ color: t.subink }}>{label}</div><div className="mt-3 text-[32px] font-semibold tabular-nums" style={{ color: t.ink, letterSpacing: '-.03em' }}>{value}</div><div className="mt-2 text-[12px]" style={{ color: t.subink }}>{note}</div></div>)}
      </div>
      <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }}>
        <div className="px-6 py-5 flex items-center justify-between gap-3 flex-wrap"><h2 className="text-[20px]"><span className="font-semibold" style={{ color: t.ink }}>Recent orders.</span>{' '}<span className="font-medium" style={{ color: t.subink }}>A clear view of what ships next.</span></h2><div className="inline-flex p-1 rounded-full" style={{ backgroundColor: t.cardSoft }}>{(['all', 'open', 'fulfilled'] as const).map((item) => <button key={item} type="button" onClick={() => setFilter(item)} className="px-3 h-7 rounded-full text-[12px] capitalize" style={{ color: filter === item ? t.ink : t.subink, backgroundColor: filter === item ? t.card : undefined, boxShadow: filter === item ? t.pillShadow : undefined, fontWeight: filter === item ? 600 : 500 }} data-testid={`button-orders-filter-${item}`}>{item}</button>)}</div></div>
        <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr style={{ borderTop: `1px solid ${t.hairline}`, borderBottom: `1px solid ${t.hairline}` }}>{['Order', 'Buyer', 'Item', 'Total', 'Status', 'Placed'].map((heading) => <th key={heading} className="px-6 py-3 text-[11px] uppercase tracking-wider font-semibold whitespace-nowrap" style={{ color: t.faint }}>{heading}</th>)}</tr></thead><tbody>{visible.map((order) => <tr key={order.id} className={t.hoverWash} style={{ borderBottom: `1px solid ${t.hairline}` }}><td className="px-6 py-4 text-[13px] font-semibold whitespace-nowrap" style={{ color: t.blue }}><button type="button" onClick={() => onAction(`${order.id} details opened.`)} data-testid={`button-order-${order.id}`}>{order.id}</button></td><td className="px-6 py-4 text-[13px] whitespace-nowrap" style={{ color: t.ink }}>{order.buyer}</td><td className="px-6 py-4 text-[13px] whitespace-nowrap" style={{ color: t.subink }}>{order.item}</td><td className="px-6 py-4 text-[13px] font-semibold" style={{ color: t.ink }}>{order.total}</td><td className="px-6 py-4"><span className="inline-flex items-center gap-1.5 text-[12.5px] font-medium whitespace-nowrap" style={{ color: order.status === 'Fulfilled' ? t.ready : t.ink }}><CheckCircle2 className="w-3.5 h-3.5" />{order.status}</span></td><td className="px-6 py-4 text-[12px] whitespace-nowrap" style={{ color: t.subink }}>{order.date}</td></tr>)}</tbody></table></div>
        {visible.length === 0 && <div className="px-6 py-12 text-center text-[14px]" style={{ color: t.subink }}>No orders in this view.</div>}
      </div>
    </div>
  );
}

function ReportsBody({ t, onAction }: { t: Theme; onAction: (msg: string) => void }) {
  const [metric, setMetric] = useState<'sales' | 'plays'>('sales');
  const values = metric === 'sales' ? [18, 23, 21, 31, 28, 38, 44, 36, 52, 47, 61, 57] : [28, 37, 31, 48, 42, 58, 64, 55, 72, 69, 83, 76];
  const points = values.map((v, i) => `${24 + i * 48},${205 - v * 2.1}`).join(' ');
  return (
    <div className="flex flex-col gap-5" data-testid="artist-reports-body">
      <div className="flex items-end justify-between gap-4 flex-wrap"><div><h1 className="text-[30px] font-semibold" style={{ color: t.ink, letterSpacing: '-.025em' }}>Reports.</h1><p className="mt-1 text-[14px]" style={{ color: t.subink }}>A practical read on performance, reach, and acquisition.</p></div><div className="flex items-center gap-2"><RangeSwitcher value="30d" onChange={() => {}} t={t} /><button type="button" onClick={() => onAction('Performance report downloaded.')} className="inline-flex items-center gap-2 h-9 px-4 rounded-full text-[13px] font-medium" style={{ border: `1px solid ${t.hairline}`, color: t.ink }} data-testid="button-download-report"><Download className="w-3.5 h-3.5" />Download</button></div></div>
      <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>{[['Total reach', '18.4k', '+14.2%'], ['Conversion', '1.2%', '+0.3 pts'], ['Revenue', '$4,862', '+12.7%'], ['Top market', 'United States', '46% of listeners']].map(([label, value, detail]) => <div key={label} className="rounded-2xl p-5" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }}><div className="text-[13px]" style={{ color: t.subink }}>{label}</div><div className="mt-3 text-[29px] font-semibold tabular-nums truncate" style={{ color: t.ink, letterSpacing: '-.03em' }}>{value}</div><div className="mt-2 text-[12px]" style={{ color: detail.startsWith('+') ? t.ready : t.subink }}>{detail}</div></div>)}</div>
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5"><div className="xl:col-span-2 rounded-2xl p-6" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }}><div className="flex items-center justify-between gap-3 flex-wrap mb-4"><h2 className="text-[20px]"><span className="font-semibold" style={{ color: t.ink }}>Performance.</span>{' '}<span className="font-medium" style={{ color: t.subink }}>What is moving.</span></h2><div className="inline-flex p-1 rounded-full" style={{ backgroundColor: t.cardSoft }}>{(['sales', 'plays'] as const).map((item) => <button key={item} type="button" onClick={() => setMetric(item)} className="px-3 h-7 rounded-full text-[12px] capitalize" style={{ color: metric === item ? t.ink : t.subink, backgroundColor: metric === item ? t.card : undefined, boxShadow: metric === item ? t.pillShadow : undefined, fontWeight: metric === item ? 600 : 500 }} data-testid={`button-report-${item}`}>{item}</button>)}</div></div><div className="min-h-[260px]"><svg viewBox="0 0 580 235" className="w-full h-full" preserveAspectRatio="none" aria-label={`${metric} report`} >{[35, 80, 125, 170, 215].map((y) => <line key={y} x1="24" x2="565" y1={y} y2={y} stroke={t.hairline} strokeDasharray="3 3" />)}<polyline points={points} fill="none" stroke={BLUE} strokeWidth="2.5" vectorEffect="non-scaling-stroke" />{['Aug 1', 'Aug 7', 'Aug 14', 'Aug 21', 'Aug 30'].map((x, i) => <text key={x} x={24 + i * 133} y="232" fontSize="10" fill={t.faint}>{x}</text>)}</svg></div></div><div className="rounded-2xl p-6" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }}><h2 className="text-[20px]"><span className="font-semibold" style={{ color: t.ink }}>Acquisition.</span>{' '}<span className="font-medium" style={{ color: t.subink }}>The paths to your work.</span></h2><div className="mt-5 space-y-4">{MOCK_AUDIENCE_SOURCES.map((source) => <div key={source.label}><div className="flex justify-between text-[13px]"><span style={{ color: t.ink }}>{source.label}</span><span style={{ color: t.subink }}>{source.share}%</span></div><div className="mt-2 h-1.5 rounded-full" style={{ backgroundColor: t.cardSoft }}><div className="h-full rounded-full" style={{ width: `${source.share}%`, backgroundColor: BLUE }} /></div></div>)}</div><button type="button" onClick={() => onAction('Audience report opened.')} className="mt-6 text-[13px] font-medium" style={{ color: t.blue }} data-testid="button-report-audience">View audience detail</button></div></div>
    </div>
  );
}
  ...
  return <div className="mt-5 overflow-hidden rounded-2xl" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }} data-testid="player-art-panel">
    <div className="grid grid-cols-1 md:grid-cols-[minmax(280px,420px)_1fr]">
      <div className="aspect-square overflow-hidden" style={{ backgroundColor: t.cardSoft }}>
        {release.cover ? <img src={release.cover} alt={`${release.title} player artwork`} className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center"><Image className="h-9 w-9" style={{ color: t.faint }} /></div>}
      </div>
...
function PlayerAudioPanel({ t }: { t: Theme }) {
  ...
  return <div className="mt-5 space-y-7" data-testid="player-audio-panel">
    <section className="overflow-hidden rounded-2xl" style={{ border: `1px solid ${t.hairline}` }}>
      <OtisTracksInteractive embedded dark={isDark} />
    </section>
    <section>
...
export function GoodDeedAssetPanel({ t, release }: { t: Theme; release: Pick<AdminRelease, 'cover' | 'title'> }) {
  const [format, setFormat] = useState<'print' | 'social' | 'texting'>('print');
  const [paperSize, setPaperSize] = useState<'letter' | 'a4'>('letter');
  const [printKind, setPrintKind] = useState<'free' | 'signed'>('free');
  const [socialSize, setSocialSize] = useState<'square' | 'portrait' | 'story'>('square');
  const pdf = goodDeedPrintPdfs[paperSize];
  const printPreview = goodDeedPrintPreviews[paperSize];
  const socialPreview = {
    square: goodDeedOtisSquare,
    portrait: goodDeedOtisPortrait,
    story: goodDeedOtisStory,
  }[socialSize];
  const dimensions = paperSize === 'letter' ? '8.5 × 11 in · 612 × 792 pt' : '210 × 297 mm · 595.28 × 841.89 pt';
  const formatDimensions = format === 'print'
    ? dimensions
    : format === 'texting'
      ? '1200 × 840 px'
      : socialSize === 'square'
  ...
  return <div data-testid="gooddeed-assets">
    <div>
      <div className="inline-flex gap-1 rounded-full p-1" style={{ backgroundColor: t.cardSoft }} role="tablist" aria-label="GoodDeed asset format">
        {([
          ['print', 'Print'],
          ['social', 'Social'],
          ['texting', 'Texting'],
        ] as const).map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={format === value} onClick={() => setFormat(value)} className="h-8 rounded-full px-3 text-[12px] font-medium" style={{ backgroundColor: format === value ? t.card : 'transparent', boxShadow: format === value ? t.pillShadow : undefined, color: format === value ? t.ink : t.subink }} data-testid={`tab-gooddeed-format-${value}`}>{label}</button>)}
      </div>
      <p className="mt-2 text-[11.5px]" style={{ color: t.subink }}>{format === 'social' ? 'Verified Otis production actual. Renderer preserved unchanged; representative owner, serial, and release data.' : 'GoodStudio reference preview. This format was not part of the September Otis social return.'}</p>
    </div>
    {format === 'print' && <div className="mt-4 flex flex-wrap gap-2">
      <div className="inline-flex gap-1 rounded-full p-1" style={{ backgroundColor: t.cardSoft }} role="tablist" aria-label="GoodDeed paper size">
        {([['letter', 'US Letter'], ['a4', 'A4']] as const).map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={paperSize === value} onClick={() => setPaperSize(value)} className="h-8 rounded-full px-3 text-[12px] font-medium" style={{ backgroundColor: paperSize === value ? t.card : 'transparent', boxShadow: paperSize === value ? t.pillShadow : undefined, color: paperSize === value ? t.ink : t.subink }} data-testid={`tab-gooddeed-${value}`}>{label}</button>)}
      </div>
      <div className="inline-flex gap-1 rounded-full p-1" style={{ backgroundColor: t.cardSoft }} role="tablist" aria-label="GoodDeed print edition">
        {([['free', 'Free digital'], ['signed', 'Signed print']] as const).map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={printKind === value} disabled={value === 'signed'} title={value === 'signed' ? 'Awaiting artist-specific Otis export' : undefined} onClick={() => setPrintKind(value)} className="h-8 rounded-full px-3 text-[12px] font-medium disabled:cursor-not-allowed disabled:opacity-35" style={{ backgroundColor: printKind === value ? t.card : 'transparent', boxShadow: printKind === value ? t.pillShadow : undefined, color: printKind === value ? t.ink : t.subink }}>{label}</button>)}
      </div>
    </div>}
    {format === 'social' && <div className="mt-4 inline-flex flex-wrap gap-1 rounded-2xl p-1" style={{ backgroundColor: t.cardSoft }} role="tablist" aria-label="GoodDeed social size">
      {([['square', 'Square'], ['portrait', 'Portrait'], ['story', 'Story']] as const).map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={socialSize === value} onClick={() => setSocialSize(value)} className="h-8 rounded-full px-3 text-[12px] font-medium" style={{ backgroundColor: socialSize === value ? t.card : 'transparent', boxShadow: socialSize === value ? t.pillShadow : undefined, color: socialSize === value ? t.ink : t.subink }}>{label}</button>)}
    </div>}
    <div className="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-[minmax(360px,1.15fr)_minmax(280px,.85fr)]">
      <div className="flex min-h-[520px] items-center justify-center overflow-hidden rounded-2xl p-7" style={{ backgroundColor: t.cardSoft, border: `1px solid ${t.hairline}` }}>
        <div className="relative max-w-full overflow-hidden" style={{ boxShadow: '0 12px 36px rgba(0,0,0,.12)' }}>
          {format === 'print'
            ? <img src={printPreview} alt={`${paperSize === 'letter' ? 'US Letter' : 'A4'} ${printKind === 'free' ? 'free digital' : 'signed print'} GoodDeed certificate preview`} className="max-h-[720px] max-w-full object-contain" />
            : format === 'social'
              ? <img src={socialPreview} alt={`Verified Otis production GoodDeed ${socialSize} actual`} className="max-h-[720px] max-w-full object-contain" />
              : release.cover && <GoodDeedArtistPreview art={release.cover} artistPhoto={niinaSoleilPhoto} artist={MOCK_PERSON.name} title={release.title} format={format} paper={paperSize} signed={printKind === 'signed'} social={socialSize} />}
        </div>
      </div>
      <div className="overflow-hidden rounded-2xl self-start" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}` }}>
        <div className="px-5 py-5" style={{ borderBottom: `1px solid ${t.hairline}` }}>
          ...
          <div className="mt-3 flex items-center gap-3">
            {release.cover ? <img src={release.cover} alt={`${release.title} album artwork`} className="h-16 w-16 rounded-xl object-cover" /> : <span className="flex h-16 w-16 items-center justify-center rounded-xl" style={{ backgroundColor: t.cardSoft }}><Image className="h-5 w-5" style={{ color: t.faint }} /></span>}
        ...
        {[
          ['Format', format === 'print' ? `${paperSize === 'letter' ? 'US Letter' : 'A4'} · ${printKind === 'free' ? 'Free digital' : 'Signed print'}` : format === 'social' ? `Social · ${socialSize}` : 'Texting / link preview'],
          ['Dimensions', formatDimensions],
          ['Certificate data', 'Sample fan owner · sample number · QR'],
          ['Output', format === 'print' ? '300-DPI PNG · generated PDF' : 'Approved PNG'],
        ...
        <div className="px-5 py-4" style={{ borderTop: `1px solid ${t.hairline}` }}>
          <p className="text-[11.5px] leading-relaxed" style={{ color: t.subink }}>{format === 'print' ? 'Free digital uses the GoodTunes logo. Signed print reserves the marked location for the holographic sticker. Fan owner and certificate number are clearly labeled sample data.' : format === 'social' ? 'Exact September 2026 production render from Otis. The card has square outer corners, a 45 px orange frame at the 1080 px base, and no safe-zone overlay.' : 'Artist and release use the selected profile. Fan owner and certificate number are sample personalized data.'}</p>
          {format === 'print' && <a href={pdf} target="_blank" rel="noreferrer" className={cn('mt-3 inline-flex h-8 items-center gap-1.5 rounded-full px-3 text-[12px] font-medium', t.hoverWash)} style={{ color: t.blue }} data-testid="link-gooddeed-pdf"><Eye className="h-3.5 w-3.5" />Open PDF preview</a>}
        </div>
      </div>
    </div>
  </div>;
...
type PackageArtworkSlot = 'cover' | 'sleeve' | 'label';
type PackageArtworkSlots = Record<PackageArtworkSlot, string | null>;

const PACKAGE_ARTWORK_SLOTS: Array<{ id: PackageArtworkSlot; label: string; size: string }> = [
...
function PackageArtworkChooser({
  t,
  art,
  onChange,
  onClose,
  ...
  const [url, setUrl] = useState('');
  const inputRef = useRef<HTMLInputElement>(null);
  const meta = PACKAGE_ARTWORK_SLOTS.find((item) => item.id === slot)!;
  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center p-5" style={{ backgroundColor: 'rgba(0,0,0,.62)', backdropFilter: 'blur(7px)' }} data-testid="modal-package-artwork">
    <div className="w-full max-w-[720px] overflow-hidden rounded-2xl" style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}`, boxShadow: '0 24px 64px rgba(0,0,0,.38)' }}>
      <div className="flex items-start justify-between gap-4 px-7 pt-6">
        <div>
          <h2 className="text-[19px] font-semibold" style={{ color: t.ink }}>Artwork · {meta.label}</h2>
          <p className="mt-1 text-[12.5px]" style={{ color: t.subink }}>Artwork stays with this artist and release.</p>
          <div className="mt-3 inline-flex items-center rounded-full p-[3px]" style={{ backgroundColor: t.cardSoft }} role="tablist" aria-label="Artwork slot">
            {PACKAGE_ARTWORK_SLOTS.map((item) => <button key={item.id} type="button" role="tab" aria-selected={slot === item.id} onClick={() => setSlot(item.id)} className="rounded-full px-3.5 py-1 text-[12px] font-medium" style={{ color: slot === item.id ? t.ink : t.faint, backgroundColor: slot === item.id ? t.card : 'transparent', border: `1px solid ${slot === item.id ? t.hairline : 'transparent'}` }} data-testid={`chip-package-art-${item.id}`}>{item.label}</button>)}
          </div>
        </div>
        <button type="button" onClick={onClose} className={cn('flex h-8 w-8 items-center justify-center rounded-full', t.hoverWash)} style={{ color: t.subink }} aria-label="Close artwork"><X className="h-4 w-4" /></button>
      </div>
      <div className="grid gap-6 px-7 pb-5 pt-5 md:grid-cols-[230px_1fr]">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: t.faint }}>Currently on the release</p>
          <div className="mt-2.5 aspect-square overflow-hidden" style={{ backgroundColor: t.cardSoft, border: `1px solid ${t.hairline}` }}>
            {current ? <img src={current} alt={`Current ${meta.label.toLowerCase()} artwork`} className="h-full w-full object-cover" /> : <div className="flex h-full flex-col items-center justify-center gap-2"><Image className="h-6 w-6" style={{ color: t.faint }} /><span className="text-[12px]" style={{ color: t.subink }}>No custom art yet</span></div>}
          </div>
          <p className="mt-3 text-center text-[11.5px]" style={{ color: t.faint }}>Suggested · {meta.size}</p>
        </div>
        <div className="flex min-h-[260px] flex-col">
          <div className="flex items-center justify-between gap-4">
            <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: t.faint }}>New artwork</p>
            <div className="inline-flex rounded-full p-[3px]" style={{ backgroundColor: t.cardSoft }} role="tablist" aria-label="Artwork source">
              <button type="button" role="tab" aria-selected={source === 'upload'} onClick={() => setSource('upload')} className="rounded-full px-3 py-1 text-[12px]" style={{ color: source === 'upload' ? t.ink : t.faint, backgroundColor: source === 'upload' ? t.card : 'transparent' }}>Upload file</button>
              <button type="button" role="tab" aria-selected={source === 'url'} onClick={() => setSource('url')} className="rounded-full px-3 py-1 text-[12px]" style={{ color: source === 'url' ? t.ink : t.faint, backgroundColor: source === 'url' ? t.card : 'transparent' }}>Paste a URL</button>
            </div>
          </div>
          {source === 'upload' ? <button type="button" onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); useFile(event.dataTransfer.files?.[0]); }} className={cn('mt-2.5 flex flex-1 flex-col items-center justify-center gap-2', t.hoverWash)} style={{ border: `1.5px dashed ${t.hairline}` }} data-testid="button-package-art-upload"><Upload className="h-5 w-5" style={{ color: t.subink }} /><span className="text-[13.5px] font-medium" style={{ color: t.ink }}>Drag a file here, or click to pick</span><span className="text-[12px]" style={{ color: t.faint }}>JPG, PNG, or PDF</span></button> : <div className="mt-2.5 flex flex-1 flex-col items-center justify-center gap-3 px-6" style={{ border: `1.5px dashed ${t.hairline}` }}><div className="flex w-full gap-2"><input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://…" className="h-9 min-w-0 flex-1 rounded-full px-3 text-[12.5px] outline-none" style={inputStyle(t)} /><button type="button" disabled={!url.trim()} onClick={() => { if (url.trim()) onChange(slot, url.trim()); }} className="h-9 rounded-full px-4 text-[12.5px] font-semibold disabled:opacity-40" style={{ backgroundColor: t.cardSoft, color: t.ink }}>Use URL</button></div><span className="text-[12px]" style={{ color: t.faint }}>Dropbox, Drive, or another direct image link</span></div>}
          <input ref={inputRef} type="file" accept=".pdf,image/*" hidden onChange={(event) => { useFile(event.currentTarget.files?.[0]); event.currentTarget.value = ''; }} />
        </div>
      </div>
      <div className="flex justify-end gap-3 px-7 pb-6">
        <button type="button" onClick={onClose} className="h-9 rounded-full px-5 text-[13px] font-medium" style={{ border: `1px solid ${t.hairline}`, color: t.subink }}>Cancel</button>
        <button type="button" onClick={onClose} className="h-9 rounded-full px-5 text-[13px] font-semibold text-white" style={{ backgroundColor: t.blue }}>Save artwork</button>
      </div>
    </div>
  </div>;
}

function AgreedPackageRecord({
  t,
  release,
  artwork,
  sleeveArtwork,
  ...
  const isRubyRecord = release.id === 'california-land' || Boolean(snapshot && /\bruby\b/i.test(`${snapshot.title} ${snapshot.subtitle}`));
  const locked = Boolean(release.packageLocked || release.paymentRecorded);
  const lockReason = release.packageLockReason
    ?? (release.paymentRecorded ? 'Locked because a payment has been recorded.' : 'Locked by the press or super-admin.');
  const quantity = snapshot?.minRun ?? 500;
  const unitCost = snapshot?.unitCost ?? 8.27;
  const setupCost = isCaliforniaLandRecord ? 1295 : 0;
  const runCost = unitCost * quantity;
  const total = runCost + setupCost;
  const packageTitle = snapshot?.title ?? 'Single LP';
  const packageSummary = snapshot?.subtitle ?? '12″ · 500 units · 140g · Translucent Ruby · Single jacket · Printed sleeve';
  const jacketArt = artwork ?? release.cover;
  ...
  return (
    <section data-testid="agreed-package-production-record">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className="text-[22px] font-semibold tracking-tight" style={{ color: t.ink }}>Package. <span className="font-normal" style={{ color: t.subink }}>The production record.</span></h2>
          <p className="mt-1 text-[13px]" style={{ color: t.subink }}>Review the finished package here. Open the full estimator to make changes while it remains unlocked.</p>
        </div>
        {locked ? (
          <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-[12.5px] font-medium" style={{ color: t.subink, border: `1px solid ${t.hairline}`, backgroundColor: t.card }}>
            <LockKeyhole className="h-3.5 w-3.5" /> Package locked
          </span>
        ) : null}
      </div>
      <div className="mt-5 overflow-hidden rounded-2xl" style={{ border: `1px solid ${t.hairline}`, backgroundColor: t.card }} data-testid="card-package-estimate-summary">
        <div className="flex items-center justify-between gap-4 px-6 py-4" style={{ borderBottom: `1px solid ${t.hairline}` }}>
          <div>
            <p className="text-[15px] font-semibold" style={{ color: t.ink }}>Your package is ready.</p>
            <p className="mt-0.5 text-[12.5px]" style={{ color: t.subink }}>{locked ? 'This production package is now read-only.' : 'Review it here, or open the estimator to make changes.'}</p>
          </div>
          <span className="text-[12px] font-medium tabular-nums" style={{ color: t.subink }}>{quantity.toLocaleString()} units</span>
        </div>
        <div className="grid grid-cols-1 gap-8 p-6 xl:grid-cols-[minmax(360px,0.9fr)_minmax(480px,1.1fr)]">
          <div className="min-w-0">
            <div className="relative mx-auto w-full max-w-[380px]" data-testid="package-final-artwork">
              <VinylPackageArtwork
                artworkUrl={jacketArt}
                sleeveArtworkUrl={sleeveArtwork ?? (isCaliforniaLandRecord ? californialandInnerSleeve : undefined)}
                labelArtworkUrl={labelArtwork}
                vinylColor={isCaliforniaLandRecord ? '#9c2633' : snapshotExplicitlyNamesBlack ? '#111111' : '#c6c8cc'}
                vinylPhotoUrl={isRubyRecord ? rubyVinylPhoto : undefined}
                onChangeArtwork={locked ? undefined : onOpenArtwork}
                testId="artist-package-canon-motion"
              />
            </div>
            <div className="mt-3">
              <p className="text-[12.5px]" style={{ color: t.subink }}>{jacketArt ? `${release.title} artwork and selected pressing` : 'Add the artist artwork when it is ready.'}</p>
            </div>
          </div>
          <div className="min-w-0">
            <p className="text-[12.5px]" style={{ color: t.subink }}>{packageTitle} · {packageSummary}</p>
            <div className="mt-4 overflow-hidden rounded-2xl" style={{ border: `1px solid ${t.hairline}` }}>
              {[
                ['Per record', 'This exact build at this run', money(unitCost)],
                ['Run', 'Pressed and packed', `${quantity.toLocaleString()} units · ${money(runCost)}`],
                ['Setup', setupCost ? 'One-time · same at any run size' : 'Included in package pricing', money(setupCost)],
              ].map(([label, detail, value], index) => <div key={label} className="flex items-center justify-between gap-5 px-5 py-4" style={{ borderTop: index ? `1px solid ${t.hairline}` : undefined }}>
                <div><p className="text-[13px] font-medium" style={{ color: t.ink }}>{label}</p><p className="mt-0.5 text-[11.5px]" style={{ color: t.subink }}>{detail}</p></div>
                <span className="text-[13px] font-semibold tabular-nums" style={{ color: t.ink }}>{value}</span>
              </div>)}
              <div className="flex items-end justify-between gap-5 px-5 py-4" style={{ borderTop: `1px solid ${t.hairline}`, background: `linear-gradient(180deg, color-mix(in srgb, ${t.blue} 10%, transparent), transparent)` }}>
                <div><p className="text-[11px] font-bold uppercase tracking-wider" style={{ color: t.blue }}>Estimate total</p><p className="mt-0.5 text-[11.5px]" style={{ color: t.subink }}>If the full run is pressed</p></div>
                <span className="text-[30px] font-semibold tracking-tight tabular-nums" style={{ color: t.ink }}>{money(total)}</span>
              </div>
            </div>
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              <p className="inline-flex items-center gap-1.5 text-[12px]" style={{ color: t.subink }}>
                {locked ? <LockKeyhole className="h-3.5 w-3.5" /> : <ShieldCheck className="h-3.5 w-3.5" />}
                {locked ? lockReason : 'Editable until the press or super-admin locks it, or a payment is recorded.'}
              </p>
              {!locked && <button type="button" onClick={onEdit} className={cn('h-10 rounded-full px-5 text-[13px] font-semibold', t.hoverWash)} style={{ color: t.ink, border: `1px solid ${t.hairline}`, backgroundColor: 'transparent' }} data-testid="button-open-package-estimator">Edit package</button>}
            </div>
          </div>
        </div>
      </div>
...
function ReleaseDetailSurface({ t, release, onSave, homeRequest, assetsRequest, onProofOpenChange }: {
  t: Theme;
  release: AdminRelease;
  onSave: (release: AdminRelease) => void;
  homeRequest: number;
  assetsRequest: number;
  ...
  const [packageArtworkOpen, setPackageArtworkOpen] = useState(false);
  ...
  return <section data-testid="release-detail-surface">
    ...
    <div className="mt-6">{tab === 'Dashboard' && <ReleaseDashboard t={t} onOpenTab={setTab} />}
      ...
      {tab === 'Assets' && <div>
        ...
        {assetFormat === 'GoodDeed®' ? <div className="mt-5"><GoodDeedAssetPanel t={t} release={release} /></div> : assetLane === 'Audio' && assetFormat === 'Vinyl' ? <VinylAudioPanel t={t} /> : assetLane === 'Audio' ? <PlayerAudioPanel t={t} /> : assetFormat === 'Vinyl' ? <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-3" data-testid="vinyl-template-grid">
            {[
              ...
              t={t}
              onReview={() => { setTemplateOpen(true); onProofOpenChange(true); }}
      ...
      {tab === 'Package' && <div data-testid="release-package">
        {release.packageState === 'agreed' && !packageEditing ? <AgreedPackageRecord
          t={t}
          release={release}
          artwork={vinylArtUploads[artworkKey('Cover · jacket')]}
          sleeveArtwork={vinylArtUploads[artworkKey('Printed inner sleeve')]}
          labelArtwork={vinylArtUploads[artworkKey('Center labels')]}
          onOpenArtwork={() => setPackageArtworkOpen(true)}
          onEdit={() => setPackageEditing(true)}
        /> : <>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h2 className="text-[22px] font-semibold tracking-tight" style={{ color: t.ink }}>{packageEditing ? 'Edit package.' : 'Build package.'} <span className="font-normal" style={{ color: t.subink }}>Configure this release in place.</span></h2>
              <p className="mt-1 text-[13px]" style={{ color: t.subink }}>The custom estimator stays inside this Artist release. Saved changes return to the package summary.</p>
            </div>
            {packageEditing && <QuietAction t={t} onClick={() => setPackageEditing(false)} testid="button-cancel-package-edit">Back to package</QuietAction>}
          </div>
          <ArtistReleasePackageBuilderContent
            embedded
            showPackages={false}
            packageState="draft"
            packageSnapshot={release.packageSnapshot ?? (release.id === 'california-land' ? {
              presetId: 'accepted-press-estimate',
              title: 'Single LP',
              subtitle: '12″ · 500 units · 140g · Translucent Ruby · Single jacket · Printed sleeve',
              minRun: 500,
              unitCost: 8.27,
              source: 'Memphis Record Pressing package builder',
            } : undefined)}
            onConvert={(snapshot) => {
              onSave({ ...release, packageState: 'agreed', packageSnapshot: snapshot });
              setPackageEditing(false);
            }}
            art={{
              cover: vinylArtUploads[artworkKey('Cover · jacket')] ?? release.cover ?? null,
              sleeve: vinylArtUploads[artworkKey('Printed inner sleeve')] ?? (release.id === 'california-land' ? californialandInnerSleeve : null),
              label: vinylArtUploads[artworkKey('Center labels')] ?? null,
            }}
            onOpenArtwork={() => setPackageArtworkOpen(true)}
          />
        </>}
        {packageArtworkOpen && <CanonPackageArtworkDialog
          open
          art={{
            cover: vinylArtUploads[artworkKey('Cover · jacket')] ?? release.cover ?? null,
            sleeve: vinylArtUploads[artworkKey('Printed inner sleeve')] ?? (release.id === 'california-land' ? californialandInnerSleeve : null),
          ...
          onChange={(slot, value) => {
            const label = slot === 'cover' ? 'Cover · jacket' : slot === 'sleeve' ? 'Printed inner sleeve' : 'Center labels';
            setVinylArtUploads((current) => ({ ...current, [artworkKey(label)]: value }));
          }}
          onClose={() => setPackageArtworkOpen(false)}
          theme={{
            card: t.card,
            raised: t.cardSoft,
            ink: t.ink,
              ...
              {editingArtistUrl ? (
                ...
                <div className="flex min-h-20 items-center gap-3 px-6 py-4">
                  <img
                    ...
                    style={{ filter: t === THEMES.dark ? 'brightness(0) invert(1)' : undefined }}
                  />
...
export function ArtistDashboardAccountStack({
  ...
  const [activeTab, setActiveTab] = useState<ArtistLiveTab>(() => {
    const requestedTab = new URLSearchParams(window.location.hash.split('?')[1] ?? '').get('tab');
    return ARTIST_LIVE_NAV.some((item) => item.id === requestedTab) || requestedTab === 'settings'
      ? requestedTab as ArtistLiveTab
      : 'dashboard';
  });
  ...
  const [releaseHomeRequest, setReleaseHomeRequest] = useState(0);
  const [releaseAssetsRequest, setReleaseAssetsRequest] = useState(0);
  const [releaseProofOpen, setReleaseProofOpen] = useState(false);
  ...
  const openFeedback = () => {
    document.querySelector<HTMLButtonElement>('[data-testid="feedback-enter"]')?.click();
  };
  ...
  return (
    ...
    >
      {/* ── Caller-owned header ── */}
      {(artistShell || !isSuperAdmin) ? <header
      ...
      >
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <button type="button" onClick={openFeedback} className={cn('inline-flex items-center gap-1.5 rounded-full text-[12.5px] font-medium px-3 h-8', t.hoverWash)} style={{ color: t.subink }} data-testid="button-header-feedback">
            ...
            <div className="space-y-6">
              ...
              {(!artistShell || selectedRelease) && <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex flex-wrap items-center gap-1.5 text-[13px]" style={{ color: t.faint }}>
                  ...
                  {selectedRelease && (isSuperAdmin
                    ? <><ChevronRight className="w-3.5 h-3.5" /><button type="button" onClick={() => { setSelectedRelease(null); setActiveTab('catalog'); setReleaseProofOpen(false); }} className="hover:underline" style={{ color: t.faint }}>Releases</button>{releaseProofOpen && <><ChevronRight className="w-3.5 h-3.5" /><button type="button" onClick={() => setReleaseAssetsRequest((value) => value + 1)} className="hover:underline" style={{ color: t.faint }}>Assets</button></>}<ChevronRight className="w-3.5 h-3.5" /><button type="button" onClick={() => setReleaseHomeRequest((value) => value + 1)} className="hover:underline" style={{ color: t.ink, fontWeight: 600 }}>{selectedRelease.title}</button></>
                    : <>{releaseProofOpen && <><ChevronRight className="w-3.5 h-3.5" /><button type="button" onClick={() => setReleaseAssetsRequest((value) => value + 1)} className="hover:underline" style={{ color: t.faint }}>Assets</button></>}<ChevronRight className="w-3.5 h-3.5" /><button type="button" onClick={() => setReleaseHomeRequest((value) => value + 1)} className="hover:underline" style={{ color: t.ink, fontWeight: 600 }}>{selectedRelease.title}</button></>
                  )}
                </div>
              ...
              {activeTab === 'catalog' && selectedRelease ? (
                <ReleaseDetailSurface
                  ...
                  homeRequest={releaseHomeRequest}
                  assetsRequest={releaseAssetsRequest}
                  onProofOpenChange={setReleaseProofOpen}
                  onSave={(updated) => { setReleases((current) => current.map((item) => item.id === updated.id ? updated : item)); setSelectedRelease(updated); setMessage('Release details saved'); }}
                />
              ) : activeTab === 'catalog' && (
                <ReleasesWall
                  t={t}
              ...
              {activeTab === 'audience' && <AudienceBody t={t} onAction={setMessage} />}
              {activeTab === 'orders' && <OrdersBody t={t} onAction={setMessage} />}
              {activeTab === 'reports' && <ReportsBody t={t} onAction={setMessage} />}
            </div>
          </div>
        </main>
      </div>
