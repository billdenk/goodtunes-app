...
import californialandProofPage from '../assets/californialand-jacket-proof-page.webp';
import mrpJacketTemplate from '../assets/mrp-jacket-proof-template.png';
...
const MOCK_ART = {
  ...
  image: californialandProofPage,           // full production PDF page
  alt: 'CALIFORNIALAND 12-inch jacket spread seated in the press template',
};
const MRP_JACKET_ASPECT = 779.41 / 539.33;
...
export function ArtistTemplateTest({
  ...
  const [templateOpacity, setTemplateOpacity] = useState(55);
  ...
  const workspace = (
      <div className="mx-auto w-full" style={{ maxWidth: 1080, padding: embedded ? '8px 0 96px' : '32px 40px 96px' }} data-testid="artist-template-test">
        ...
        {showNavigation && <nav aria-label="Template navigation" data-testid="breadcrumb" className="flex flex-wrap items-center gap-3">
          {embedded
            ? <button type="button" onClick={onBack} className={cn('inline-flex h-9 items-center gap-1.5 rounded-full px-3 text-[12.5px] font-semibold', t.hoverCard)} style={{ color: t.ink, border: `1px solid ${t.hairline}`, backgroundColor: t.card }} data-testid="link-back-assets"><ArrowLeft className="size-3.5" />Back to Assets</button>
            : <a href="#/ArtistPortalRestructureFlow" className={cn('inline-flex h-9 items-center gap-1.5 rounded-full px-3 text-[12.5px] font-semibold', t.hoverCard)} style={{ color: t.ink, border: `1px solid ${t.hairline}`, backgroundColor: t.card }} data-testid="link-back-assets"><ArrowLeft className="size-3.5" />Back to Assets</a>}
          <ol className="flex flex-wrap items-center gap-2 text-[12.5px]" style={{ color: t.faint }}>
            <li>Vinyl art</li>
            <li aria-hidden><ChevronRight className="size-3.5" /></li>
            <li><span aria-current="page" style={{ color: t.subink }}>{MOCK_ART.title}</span></li>
          </ol>
        </nav>}
        ...
        <h1 style={{ marginTop: showNavigation ? 12 : 0, fontSize: 30, letterSpacing: '-0.02em', fontWeight: 600, lineHeight: 1.12 }}>
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap" style={{ marginTop: 14 }}>
          <div className="flex items-center gap-2 flex-wrap" data-testid="overlay-chips">
            {OVERLAY_CHIPS.map((c) => c.id === 'template' ? (
              <div key={c.id} className="relative flex-shrink-0">
                <span
                  className="inline-flex h-7 items-center overflow-hidden rounded-full transition-colors"
                  style={{
                    border: `1px solid ${templateOn ? t.subink : t.chipBorder}`,
                    backgroundColor: templateOn ? t.soft : 'transparent',
                  }}
                >
                  <button
                    type="button"
                    onClick={() => setTemplateOn((current) => {
                      const next = !current;
                      setTemplatePanelOpen(next);
                      return next;
                    })}
                    aria-pressed={templateOn}
                    className="inline-flex h-full items-center gap-1.5 pl-2.5 pr-1.5 text-[12px] font-medium"
                    style={{ color: templateOn ? t.ink : t.faint }}
                    data-testid="overlay-template"
                  >
                    <span className="h-2 w-2 flex-shrink-0 rounded-full" style={{ backgroundColor: templateOn ? t.subink : t.faint }} />
                    Template
                    <span style={{ color: t.faint }}>{templateOn ? 'on' : 'off'}</span>
                  </button>
                  <button
                    type="button"
                    ...
                    className="inline-flex h-full items-center pl-0.5 pr-2"
                    style={{ color: t.faint }}
                ...
                {templatePanelOpen && (
                  <>
                    ...
                    <div
                      className="absolute left-0 z-[61] w-[210px] rounded-xl px-4 py-3 shadow-2xl"
                      style={{ backgroundColor: t.card, border: `1px solid ${t.hairline}`, top: 'calc(100% + 6px)' }}
                      role="dialog"
                      aria-label="Template display options"
                    ...
                    >
                      ...
                      <label className="block text-[11px] font-semibold" style={{ color: t.subink }}>
                        ...
                        <input
                          ...
                          onChange={(event) => setTemplateOpacity(Number(event.target.value))}
                          className="gt-slider mt-2 block w-full"
        ...
        >
          {hasArt ? (
            <div
              ...
              style={{ maxWidth: `${zoom}%`, aspectRatio: `${MRP_JACKET_ASPECT}`, transition: 'max-width 0.1s linear' }}
              data-testid="mrp-proof-sheet"
            >
              <img
                src={MOCK_ART.image}
                alt={MOCK_ART.alt}
              ...
              {templateOn && (
                <img
                  src={mrpJacketTemplate}
                  alt="Memphis Record Pressing 12-inch single 3D jacket template"
                  className="pointer-events-none absolute inset-0 h-full w-full object-contain"
                  style={{ opacity: templateOpacity / 100, mixBlendMode: 'multiply' }}
                  draggable={false}
                  data-testid="canvas-template-overlay"
                />
              )}
            </div>
          ) : (
