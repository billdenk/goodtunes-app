// PressClientEstimateEmail — the EMAIL the platform pushes out when a press
// sends a client their estimate. Companion to PressClientEstimate (the live
// page the button opens). Bill's ask (Aug 20 2026): "what an email version of
// this would look like for the platform to push out."
//
...
export default function PressClientEstimateEmail() {
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0b', color: INK, fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Segoe UI', sans-serif", padding: '40px 16px 80px' }}>

      {/* ── Inbox chrome (mock-only) — who it's from and the subject ── */}
      <div style={{ maxWidth: 600, margin: '0 auto 14px', padding: '14px 18px', borderRadius: 14, background: CARD, border: `1px solid ${HAIRLINE}` }} data-testid="email-chrome">
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', borderRadius: 18, overflow: 'hidden', background: CANVAS, border: `1px solid ${HAIRLINE}` }} data-testid="email-body">
        <div style={{ padding: '36px 36px 40px' }}>
          ...
          <div style={{ marginTop: 30, display: 'flex', justifyContent: 'center' }}>
            <div style={{ position: 'relative', width: 360, height: 232 }} aria-hidden data-testid="email-album-stage">
              ...
              <img
                ...
                alt=""
                style={{ position: 'absolute', left: 0, top: 0, width: 226, height: 226, borderRadius: 4, objectFit: 'cover', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}
              />
            </div>
          </div>

          {/* Totals card — everything expanded; an email can't collapse */}
          <div style={{ marginTop: 30, borderRadius: 14, border: `1px solid ${HAIRLINE}`, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '16px', borderTop: `1px solid ${HAIRLINE}`, background: `linear-gradient(180deg, ${BLUE_TINT_TOP} 0%, ${CARD} 100%)` }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 1.2, textTransform: 'uppercase', color: BLUE }}>Estimate total</div>
          ...
          <div style={{ marginTop: 26, textAlign: 'center' }}>
            <a
              ...
              style={{
                display: 'inline-block', padding: '13px 34px', borderRadius: 999,
                background: BLUE, color: '#fff', fontSize: 14.5, fontWeight: 600, textDecoration: 'none',
              }}
            >
              Open your estimate
            </a>
          ...
          <div style={{ marginTop: 22, textAlign: 'center', fontSize: 13 }}>
            ...
            <a href="#" onClick={(e) => e.preventDefault()} style={{ color: BLUE, textDecoration: 'none' }} data-testid="email-goodtunes-hook">Learn more →</a>
          </div>

          {/* Brandon sign-off — replaces the page's "Ask a question" sheet;
              in email, the reply button IS the question channel. */}
          <div style={{ marginTop: 30, padding: '14px 16px', borderRadius: 14, background: CARD, border: `1px solid ${HAIRLINE}`, display: 'flex', alignItems: 'center', gap: 14 }}>
            <span style={{ width: 44, height: 44, borderRadius: 12, overflow: 'hidden', flexShrink: 0, border: `1px solid ${HAIRLINE}` }}>
        ...
        <div style={{ padding: '24px 36px 30px', background: CARD, borderTop: `1px solid ${HAIRLINE}`, textAlign: 'center' }}>
          <img src={mrpLogoAsset} alt={MOCK_PRESS} style={{ width: 34, height: 34, filter: 'brightness(0) invert(1)', opacity: 0.9 }} />
