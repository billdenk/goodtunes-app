...
// panel → expanded view. Edit this file once — all tabs update (no drift).
...
function StudyThumb({ spec, panel, zone, mode, flipped, onFlip, fixed, onFix, size = 300, onExpand, flagDecision, flagOpen, onFlagToggle, onFlagFix, onFlagAccept, onFlagReview, onFlagReset }: { spec: StudySpec; panel: StudyPanel; zone: string | null; mode: ViewMode; flipped: boolean; onFlip?: () => void; fixed?: boolean; onFix?: () => void; size?: number; onExpand?: () => void; flagDecision?: Decision; flagOpen?: boolean; onFlagToggle?: () => void; onFlagFix?: () => void; onFlagAccept?: (reason: string) => void; onFlagReview?: (recipient: string) => void; onFlagReset?: () => void }) {
  ...
  return (
    <div className="relative flex flex-col items-center" data-testid={`study-thumb-${panel.label.toLowerCase().replace(/ /g, '-')}`}>
      ...
      {/* Invisible hover bridge: the icon strip sits 26px outside the panel
          box, so without this the icons fade the moment the cursor crosses
          the gap toward them. */}
      {(onExpand || onFlip) && <span aria-hidden className="absolute inset-y-0 -right-7 w-7" />}
      {onExpand && (
        <button
          type="button"
          onClick={onExpand}
          ...
          className="absolute z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 hover:opacity-100 transition-opacity"
          style={{ top: 2, right: -26, width: 20, height: 20, color: t.subink, background: 'transparent' }}
          data-testid={`button-expand-${panel.label.toLowerCase().replace(/ /g, '-')}`}
        >
      ...
      <button
        type="button"
        onClick={onExpand}
        className={`relative overflow-hidden w-full h-full ${circle ? 'rounded-full' : ''} ${onExpand ? 'cursor-zoom-in transition-transform hover:scale-[1.02]' : 'cursor-default'}`}
        style={{ backgroundColor: '#fff', border: circle ? `1px solid ${t.hairline}` : 'none' }}
...
export default function PrintedAreasStudy({ spec, embedded, panelSize, headerAction, theme = STUDY_DARK }: { spec: StudySpec; embedded?: boolean; panelSize?: number; headerAction?: React.ReactNode; theme?: StudyTheme }) {
  const t = theme;
