...
function StatusBadge({
  ...
  if (compact) {
    return (
      <button
        ...
        onClick={onClick}
        className={[
          "group/card flex flex-col items-center justify-center gap-1.5 px-3 py-3 rounded-xl bg-white border text-center w-full transition-all relative",
          active
      ...
      >
        <span
          className={[
            "w-8 h-8 rounded-full inline-flex items-center justify-center flex-shrink-0 relative",
        ...
        <ChevronDown
          className={[
            "w-3.5 h-3.5 text-slate-400 transition-transform absolute top-1.5 right-1.5",
            active ? "rotate-180" : "",
          ].join(" ")}
        />
      </button>
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={[
        "group/card flex items-center justify-between gap-2 rounded-xl bg-white border text-left w-full transition-all",
    ...
    >
      ...
      {/* Disclosure is always a chevron; completion remains in the status icon. */}
      <span
        className={[
...
function DetailWrap({
  ...
  return (
    <div className="relative mt-4 rounded-2xl border border-slate-200 bg-white p-5 space-y-5">
...
function MasterDetail({
  ...
  const downloadMockMaster = () => {
    const blob = new Blob(
      ["GoodTunes local mock master download — no production audio is included."],
      { type: "audio/wav" },
    );
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = "storms_master_24-96.mock.wav";
    ...
    <DetailWrap title="Master" onClose={onClose}>
      {hasMaster ? (
        ...
        >
          ...
          <button
            type="button"
            onClick={downloadMockMaster}
            aria-label="Download a local mock of storms_master_24-96.wav"
            className="w-8 h-8 rounded-full inline-flex items-center justify-center flex-shrink-0 text-slate-400 hover:bg-slate-100 hover:text-[#319ED8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#319ED8]/40"
          >
  ...
  return (
    ...
    >
      ...
      {locked && committedLeft < 0.5 ? (
        ...
        <div className="-mt-1 rounded-xl bg-[var(--apple-chip)] border border-[var(--apple-hairline)] px-3 py-2.5 flex items-start gap-2.5">
          <span className="w-7 h-7 rounded-lg bg-[var(--apple-track)] text-[#319ED8] inline-flex items-center justify-center flex-shrink-0">
          ...
          <div className="text-[11.5px] leading-snug flex-1 min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2">
              <button
                onClick={saveAndLock}
                className="inline-flex h-8 items-center gap-1 rounded-full px-3 text-[11.5px] font-semibold bg-[#319ED8] text-white hover:bg-[#319ED8]/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#319ED8]/40"
            ...
            <div
              ...
              className={[
                ...
                locked
                  ...
                  : "border-[var(--apple-sev-warning)] bg-[var(--apple-sev-warning-wash)] cursor-grab active:cursor-grabbing shadow-[0_0_0_3px_color-mix(in_srgb,var(--apple-sev-warning)_15%,transparent)]",
              ].join(" ")}
...
function LyricsDetail({
  ...
  // 'plain' = no timing · 'syncing' = forced-alignment call in flight · 'synced' = LyricFlow™ (word-level VTT stored)
  const [syncState, setSyncState] = useState<"plain" | "syncing" | "synced">(
    "plain",
  );
  // Once LyricFlow™ is added, the synced view becomes the source of truth — each line
  ...
  // LyricFlow™ badge — same chip family as Dolby Atmos / Lossless / SuperCredits™.
  // Brand-blue solid pill, mic glyph + check, used here in the card header and
  // intended to graduate to album covers / song rows / library filter ("Albums with LyricFlow™").
  const lyricFlowBadge = (
    <span
  ...
  const titleAction =
    syncState === "synced" ? (
      <div className="flex items-center gap-1.5">{lyricFlowBadge}{importAction}</div>
    ) : (
      importAction
    );
  ...
  return (
    <DetailWrap title="Lyrics" onClose={onClose} action={titleAction}>
      {instrumental ? (
        <div className="rounded-xl border border-[var(--apple-hairline)] bg-[var(--apple-track)] p-4 flex items-start gap-3">
          <span className="w-8 h-8 rounded-lg bg-[var(--apple-chip)] text-[var(--apple-subink)] inline-flex items-center justify-center flex-shrink-0" aria-hidden>
            <FileText className="w-3.5 h-3.5" />
          </span>
          <div className="min-w-0">
            <div className="text-[12.5px] font-semibold text-[var(--apple-ink)]">Lyrics aren't applicable.</div>
            <div className="text-[11.5px] text-[var(--apple-subink)] mt-0.5">
              This track is marked Instrumental in the Master editor. Uncheck Instrumental on the master to add lyrics.
            </div>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="flex min-w-0 flex-col gap-2">
            <div className="flex h-7 items-center justify-between">
              <h4 className="text-[13px] font-semibold text-[var(--apple-ink)]">Editable Lyrics</h4>
              <span className="text-[10px] text-[var(--apple-faint)]">{text ? text.split("\n").length : 0} lines</span>
            </div>
            <textarea
              value={text}
              onChange={(e) => setText(e.target.value)}
              rows={9}
              placeholder="Paste or import lyrics."
              className="min-h-[200px] w-full resize-none rounded-xl border border-[var(--apple-hairline)] bg-[var(--apple-card)] px-3 py-3 font-mono text-[12.5px] leading-relaxed text-[var(--apple-ink)] outline-none focus:border-[#319ED8] focus:ring-2 focus:ring-[#319ED8]/20"
            />
            <div className="flex items-center justify-between text-[10.5px] text-[var(--apple-faint)]"><button className="rounded-full px-2.5 py-1 hover:bg-[var(--apple-track)]">View original</button><span>Autosaves after editing</span></div>
          </div>
          <div className="flex min-w-0 flex-col gap-2">
            <div className="flex h-7 items-center justify-between">
              <h4 className="text-[13px] font-semibold text-[var(--apple-ink)]">LyricFlow™</h4>
              {syncState === "synced" && lyricFlowBadge}
            </div>
            <div className="flex min-h-[200px] flex-col rounded-xl border border-[var(--apple-hairline)] bg-[var(--apple-track)] p-4">
              {syncState === "synced" ? <div className="flex-1 space-y-1.5 overflow-hidden">
                {cues.map((cue) => <div key={cue.id} className="flex gap-3 rounded-lg px-2 py-1.5 text-[11.5px] hover:bg-[var(--apple-chip)]"><span className="w-12 tabular-nums text-[var(--apple-faint)]">{cue.isHeader ? "—" : fmtTime(cue.timeMs)}</span><span className="text-[var(--apple-ink)]">{cue.text}</span></div>)}
              </div> : <div className="flex flex-1 flex-col items-center justify-center text-center">
                <Mic className="h-5 w-5 text-[var(--apple-faint)]" />
                <p className="mt-2 text-[12px] font-semibold text-[var(--apple-ink)]">Line lyrics up with the master audio.</p>
                <p className="mt-1 max-w-[240px] text-[11px] text-[var(--apple-subink)]">LyricFlow™ creates timed cues for the listener experience.</p>
              </div>}
              <div className="mt-3 flex justify-center">
                <button onClick={syncState === "synced" ? handleResync : hasText ? handleAutoSync : handleTranscribe} className="inline-flex h-9 items-center gap-1.5 rounded-full bg-[#319ED8] px-4 text-[11.5px] font-semibold text-white hover:bg-[#2890c8]">
                  {syncState === "syncing" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Mic className="h-3.5 w-3.5" />}
                  {syncState === "syncing" ? "Syncing…" : syncState === "synced" ? "Re-sync with audio" : "Sync with audio"}
                </button>
              </div>
            </div>
          </div>
...
function SplitsDetail({
  ...
  const updateParticipant = (
    id: number,
    patch: Partial<SplitParticipant>,
  ) => {
    onChange(
  ...
  return (
    <DetailWrap
      ...
      action={
        <button
          ...
          className="px-2.5 py-1.5 rounded-full text-[11.5px] font-semibold inline-flex items-center gap-1 text-[#319ED8] hover:bg-[#319ED8]/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#319ED8]/40"
        >
    ...
    >
      <p className="text-[11.5px] text-slate-500 -mt-1">
        Set each participant’s share. Changes stay in this local workspace.
      </p>
        ...
        {participants.map((participant) => (
          ...
          >
            <input
              ...
              className="min-w-0 flex-1 rounded-md border border-slate-200 bg-white px-2.5 py-1.5 text-[12px] font-medium text-slate-900 focus:border-[#319ED8] focus:outline-none focus:ring-2 focus:ring-[#319ED8]/20"
            />
            <div className="relative w-24 flex-shrink-0">
              <input
                ...
                className="w-full rounded-md border border-slate-200 bg-white py-1.5 pl-2.5 pr-7 text-right text-[12px] tabular-nums text-slate-900 focus:border-[#319ED8] focus:outline-none focus:ring-2 focus:ring-[#319ED8]/20"
              />
              <Percent className="pointer-events-none absolute right-2 top-2 w-3 h-3 text-slate-400" />
            </div>
            <button
              type="button"
              onClick={() =>
              ...
              className="w-7 h-7 rounded-full inline-flex items-center justify-center text-slate-400 hover:bg-slate-200 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-40"
            >
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
      ...
      >
        <span className="font-semibold">
          {totalIsValid ? "Total is ready" : total < 100 ? `${100 - total}% unassigned` : `${total - 100}% over`}
        </span>
...
function SegmentedControl({
  ...
  return (
    <div
      ...
      className="inline-flex h-10 rounded-full bg-muted p-1 text-[12px] font-medium"
    >
      <button
        role="tab"
        ...
        className={[
          "inline-flex h-8 items-center gap-1.5 rounded-full px-3 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
          mode === "edit"
...
type Section = "master" | "snippet" | "lyrics" | "credits" | "splits" | null;
...
function EditRow({
  ...
  const [splits, setSplits] = useState<SplitParticipant[]>([
    { id: 1, name: "GoodTunes Artist", percentage: 100 },
  ]);
  ...
  const activeOptionalStatus = openSection === "snippet"
      ? { ok: true, icon: Headphones, label: "Preview", severity: "soft" as const }
      : openSection === "lyrics"
        ? { ok: lyricsDone, icon: FileText, label: "Lyrics", subtitle: instrumental ? "Instrumental" : undefined, severity: "soft" as const }
        : openSection === "credits"
          ? { ok: t.credits, icon: Users, label: "Credits", severity: "soft" as const }
          : openSection === "splits"
            ? { ok: splits.reduce((sum, participant) => sum + participant.percentage, 0) === 100, icon: Percent, label: "Splits", severity: "soft" as const }
          : null;
  return (
    <li
      className={[
        ...
        expanded
          ? dark ? "bg-[var(--apple-card)]" : "bg-slate-50/80"
          : dark ? "hover:bg-[var(--apple-track)]" : "hover:bg-slate-50/70",
        ...
        <div className="flex-1 min-w-0">
          {expanded ? (
            <input
              ...
              autoFocus
              className={[
                "w-full px-2 -mx-2 py-0.5 rounded-md text-slate-900 text-[13.5px] font-medium bg-transparent border border-transparent hover:border-slate-200 focus:border-[#319ED8] focus:outline-none focus:ring-2 focus:ring-[#319ED8]/20",
                dark ? "focus:bg-[var(--apple-track)]" : "focus:bg-white",
              ].join(" ")}
            />
          ) : (
            <button
      ...
      {expanded && (
        <div className="pl-20 pr-16 pb-5 -mt-1 space-y-4">
          {/* 1-over-3 layout (Bill's pick):
              ...
              OPTIONAL group → Preview · Lyrics · Credits · Splits share one row
          ...
          <div className="space-y-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Required</span>
                <span className="h-px flex-1 bg-slate-200" aria-hidden />
              </div>
              <StatusBadge ok={t.master} icon={Disc3} label="Master" subtitle={t.master ? "Uploaded · tap to replace" : "Required to publish"} severity="required" size="emphasized" active={openSection === "master"} onClick={() => toggleSection("master")} />
              {openSection === "master" && (
                <MasterDetail hasMaster={t.master} onGoToSnippet={() => setOpenSection("snippet")} onClose={() => setOpenSection(null)} />
              )}
            </div>
            ...
            <div>
              ...
              {activeOptionalStatus ? (
                <StatusBadge {...activeOptionalStatus} active onClick={() => setOpenSection(null)} />
                ...
                />
                <StatusBadge
                  ok={splits.reduce((sum, participant) => sum + participant.percentage, 0) === 100}
                  icon={Percent}
                  label="Splits"
                  severity="soft"
                  compact
                  active={openSection === "splits"}
                  onClick={() => toggleSection("splits")}
                />
              </div>}
              {openSection === "snippet" && (
                <SnippetDetail hasMaster={t.master} hasCustomSnippet={t.snippet} onGoToMaster={() => setOpenSection("master")} onClose={() => setOpenSection(null)} />
              )}
              {openSection === "lyrics" && (
                <LyricsDetail hasLyrics={t.lyrics} instrumental={instrumental} onToggleInstrumental={() => setInstrumental((v) => !v)} onClose={() => setOpenSection(null)} />
              )}
              {openSection === "credits" && (
                <CreditsDetail hasCredits={t.credits} onClose={() => setOpenSection(null)} />
              )}
              {openSection === "splits" && (
                <SplitsDetail participants={splits} onChange={setSplits} onClose={() => setOpenSection(null)} />
              )}
            </div>
          </div>
...
export function Interactive({ embedded = false, dark = false, alignModeWithHeading = false }: { embedded?: boolean; dark?: boolean; alignModeWithHeading?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!root) return;
    const syncTheme = () =>
      setInheritedDark(Boolean(root.closest(".dark, .gt-admin-dark")));
    syncTheme();
  ...
  const effectiveDark = dark || inheritedDark;

  return (
    <div
      ref={rootRef}
      className={embedded ? `otis-tracks-embedded relative w-full font-[Inter,system-ui,-apple-system,sans-serif]${effectiveDark ? " otis-tracks-dark" : ""}` : "min-h-screen bg-slate-50 p-10 font-[Inter,system-ui,-apple-system,sans-serif]"}
      style={embedded && effectiveDark ? {
        "--apple-canvas": "#161617",
        "--apple-card": "#1e1e20",
        ...
        "--apple-track": "#26262a",
        "--apple-chip": "#3a3a3e",
        "--apple-sev-warning": "#e8b04b",
        "--apple-sev-warning-wash": "rgba(232,176,75,.14)",
        "--apple-sev-ready": "#4cc98a",
        "--apple-sev-ready-wash": "rgba(76,201,138,.14)",
        backgroundColor: "transparent",
        color: "var(--apple-ink)",
      } as React.CSSProperties : undefined}
    >
      {effectiveDark && <style>{`
        .otis-tracks-dark .bg-white { background-color: var(--apple-card) !important; }
        .otis-tracks-dark .bg-slate-50,
        .otis-tracks-dark .bg-slate-50\\/80,
        .otis-tracks-dark .bg-slate-50\\/70,
        .otis-tracks-dark .bg-slate-50\\/60 { background-color: var(--apple-canvas) !important; }
        .otis-tracks-dark .bg-slate-100,
        .otis-tracks-dark .bg-slate-200,
        .otis-tracks-dark .bg-slate-300 { background-color: var(--apple-track) !important; }
        .otis-tracks-dark .text-slate-900 { color: var(--apple-ink) !important; }
        .otis-tracks-dark .text-slate-700,
        .otis-tracks-dark .text-slate-600,
        .otis-tracks-dark .text-slate-500 { color: var(--apple-subink) !important; }
        .otis-tracks-dark .text-slate-400,
        .otis-tracks-dark .text-slate-300 { color: var(--apple-faint) !important; }
        .otis-tracks-dark .border-slate-100,
        .otis-tracks-dark .border-slate-200,
        .otis-tracks-dark .border-slate-300 { border-color: var(--apple-hairline) !important; }
        .otis-tracks-dark .ring-slate-200,
        .otis-tracks-dark .ring-white { --tw-ring-color: var(--apple-hairline) !important; }
        .otis-tracks-dark .hover\\:bg-slate-100:hover,
        .otis-tracks-dark .hover\\:bg-slate-50:hover { background-color: var(--apple-track) !important; }
      `}</style>}
      <div className={embedded ? "w-full" : "max-w-3xl mx-auto"}>
        <div className={alignModeWithHeading ? "absolute -top-[68px] right-0 z-10 flex justify-end" : "mb-3 flex justify-end"}>
          <SegmentedControl mode={mode} setMode={setMode} />
        </div>
        <div className="rounded-2xl bg-white border border-slate-200 overflow-hidden">
          {/* Panel header */}
          <div className="px-6 py-5 flex items-center justify-between gap-5 border-b border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-slate-900 text-[20px] font-semibold tracking-[-0.02em]">Tracks</h2>
            ...
            <div className="flex items-center gap-2">
              {mode === "edit" && (
                <>
                  <button className="inline-flex h-9 items-center gap-1.5 rounded-full border border-slate-200 bg-transparent px-3.5 text-[12px] font-semibold text-slate-700 hover:bg-slate-100">
                    <Plus className="w-3.5 h-3.5" />
                    Add track
                  </button>
                  <div className="relative">
                    <button
                      ...
                      className="inline-flex h-9 items-center gap-1.5 rounded-full px-3.5 text-[11.5px] font-semibold text-slate-500 hover:bg-slate-100 hover:text-slate-900"
                    >
                    ...
                    {advancedOpen && <>
                      <button className="fixed inset-0 z-10 cursor-default" aria-label="Close Advanced menu" onClick={() => setAdvancedOpen(false)} />
                      <div className="absolute right-0 top-10 z-20 w-[290px] rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">
                        {[
                          ["Tracks", [["Upload multiple tracks", "Empty rows or a Dropbox link."]]],
                          ["Lyrics", [["Import lyrics from Dropbox", "PDF, Word, or text files — matched to tracks."], ["Find missing lyrics", "Looks up lyrics online for every track missing them."], ["Add LyricFlow™ to your album", "Auto-sync lyrics on every eligible track."]]],
                          ["Credits", [["Import credits", "PDF, Word, or text liner notes — matched to tracks."]]],
                          ["Splits", [["Import splits from sheet", "Google Sheet or CSV — matched to tracks by title."]]],
                        ].map(([section, items]) => <div key={section as string} className="py-1">
                          <div className="px-3 pb-1 pt-1 text-[9.5px] font-semibold uppercase tracking-[.09em] text-slate-400">{section as string}</div>
                          {(items as string[][]).map(([label, detail]) => <button key={label} type="button" onClick={() => setAdvancedOpen(false)} className="w-full rounded-xl px-3 py-2 text-left hover:bg-slate-100">
                            <span className="block text-[12px] font-semibold text-slate-900">{label}</span>
                            <span className="mt-0.5 block text-[10.5px] leading-snug text-slate-500">{detail}</span>
                          </button>)}
                        </div>)}
                      </div>
                    </>}
