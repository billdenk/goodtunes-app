...
// belongs to.
//
// Lives here (rather than routes.ts, its original home) so lower-level
// gates — requireRole / resolveReportScope in auth/roles.ts — can also
// honor a Bearer token instead of session-cookie-only. Before this moved,
// those two checked `req.session?.userId` directly with no Bearer
// fallback, so any admin whose browser holds a valid Bearer token but no
// (or a stale) session cookie would 401 on every Reports/album-dashboard
// route while the rest of the admin app — gated by routes.ts's
// `requireAdmin`, which already called this — worked fine.
export type AuthIdentity = { userId: string; kind: AuthKind };
...
export async function getAuthFromRequest(req: Request): Promise<AuthIdentity | undefined> {
