...
/* ─── Frame (top bar + left sidebar) ────────────────────────────────── */

type EntityKey = "albums" | "people" | "gear" | "vendors" | "labels";
...
function AdminFrame({
  ...
  // Sidebar counts — quiet, cached. Each entity has its own list endpoint
  // already; we just need the lengths.
  ...
  // Other entities don't have their own new-admin pages yet — deep-link
  // into the classic admin's matching tab. Pattern mirrors what we use
  // from per-album pages ("Edit in classic admin →").
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans antialiased">
      ...
      <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 flex-shrink-0">
        <Link href="/admin/albums" className="flex items-center" data-testid="link-admin-home">
...
function SidebarLink({
  ...
  return (
    ...
    >
      <Icon className={["w-4 h-4 flex-shrink-0", active ? "text-[#319ED8]" : "text-slate-400"].join(" ")} />
