  ...
  return (
    <AdminFrame active="pressing-orders">
      <div className="px-6 py-6 max-w-5xl mx-auto space-y-5">
        <AdminPageHeader
          title="Press orders."
          subtitle={`Artist-submitted "Go to Press!" requests awaiting GoodTunes review.`}
          actions={
                ...
                <button
                  ...
                  className={[
                    ...
                    status === s
                      ...
                      : "bg-white border border-[var(--apple-hairline)] text-[var(--apple-subink)] hover:bg-[var(--apple-track)]",
                  ].join(" ")}
        ...
        {isLoading ? (
          <Card className="rounded-2xl shadow-none p-10 grid place-items-center" data-testid="state-pressing-orders-loading">
            <Spinner className="w-6 h-6 text-[var(--brand-blue)] animate-spin" />
          </Card>
        ) : !rows || rows.length === 0 ? (
          <Card className="rounded-2xl shadow-none" data-testid="state-pressing-orders-empty">
            <AdminEmptyState>
              Nothing {status === "all" ? "to show" : status} right now — artist "Go to Press!" submissions land here for review.
            </AdminEmptyState>
          </Card>
        ) : (
                ...
                >
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 text-[11.5px] text-[var(--apple-subink)] uppercase tracking-wide">
                      ...
                      <div className="mt-1 text-[14px] font-semibold text-[var(--apple-ink)]">
                        ...
                        {row.albumArtist && (
                          <span className="text-[var(--apple-subink)] font-normal"> · {row.albumArtist}</span>
                        )}
                      </div>
                      <div className="mt-1 text-[12.5px] text-[var(--apple-subink)]">
                        {snap.format}
                        {snap.vinylColor && <> · {snap.vinylColor}</>}
                        {" · "}
                        {row.quantity} units
                        {" · "}
                        <span className="font-semibold tabular-nums text-[var(--apple-ink)]">{dollars(row.totalCents)}</span>
                      </div>
                    </div>
                    <ChevronDown
                      className={[
                        ...
                        <div className="mt-4 space-y-2 border-t border-[var(--apple-hairline)] pt-4">
                          ...
                          <div className="flex items-center justify-end gap-2">
                            <Button
                              ...
                              className="bg-transparent text-[var(--apple-critical)] hover:bg-[var(--apple-critical)]/10 disabled:opacity-50"
  ...
  return (
    <div className="min-w-0">
      ...
      <div
        className={[
          "truncate",
          strong ? "text-[13.5px] font-semibold text-[var(--apple-ink)]" : "text-[12.5px] text-[var(--apple-ink)]",
        ].join(" ")}
      >
        {value}
        {sub && <span className="text-[var(--apple-faint)] ml-1">({sub})</span>}
      </div>
    </div>
  );
}
