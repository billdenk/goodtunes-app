...
// Album-level "Migrate to Mux" admin action. Fires the ingest endpoint for
// every track that has an audioUrl but no muxPlaybackId yet, then polls each
// asset (via /mux-refresh) until it reports `ready` or `errored`. Renders
// inline status badges so the operator can see progress without leaving the
// page. Once ready, the player will silently swap to signed HLS — the legacy
// /objects/uploads/<uuid>.wav URL stays as a fallback.
function MigrateToMuxButton({
  albumId,
  songs,
}: {
  albumId: string;
  ...
  // Unmount-safe polling: cancelRef short-circuits the recursive setTimeout
  // chain, timeoutRef lets us synchronously clear a pending tick on unmount,
  // mountedRef gates setBusy/toast so we don't update an unmounted tree.
  ...
  const onClick = async () => {
    ...
    try {
      ...
      for (const s of pending) {
        try {
          const r = await apiRequest(
            "POST",
            `/api/admin/songs/${s.id}/mux-ingest`,
          );
          if (r.ok) ingested.push(s.id);
        } catch (err) {
          console.error("[mux-ingest]", s.id, err);
        }
      }
      if (!mountedRef.current) return;
      toast({
        title: `Mux: ingest started`,
        description: `${ingested.length} of ${pending.length} tracks sent to Mux. Polling for ready…`,
      });
      // Poll every 5s up to ~3min per batch. Each pass narrows `outstanding`
      // to only the tracks still encoding, so we stop hammering the refresh
      // endpoint for ones that already flipped to ready/errored.
      let elapsed = 0;
      let outstanding = [...ingested];
      const tick = async () => {
        ...
        for (const id of outstanding) {
          try {
            const r = await apiRequest(
              "POST",
              `/api/admin/songs/${id}/mux-refresh`,
            );
            if (r.ok) {
              const data = (await r.json()) as {
                song?: { muxStatus?: string | null };
              };
              const status = data?.song?.muxStatus;
            ...
            console.error("[mux-refresh]", id, err);
            stillPending.push(id);
          }
        }
        if (cancelRef.current) return;
      ...
      console.error("[mux-migrate]", err);
      if (mountedRef.current) {
        toast({
          title: "Mux: failed to start ingest",
  ...
  return (
    <button
      ...
      className={
        "px-2.5 py-1.5 mb-1 rounded-md text-[11.5px] font-semibold inline-flex items-center gap-1.5 flex-shrink-0 transition-colors " +
      ...
      data-testid="button-migrate-to-mux"
      title="Send each track's master to Mux for adaptive HLS streaming."
    >
      {busy ? (
