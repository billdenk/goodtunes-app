...
import { Bell, Send, MailCheck, Undo2, Clock3 } from "lucide-react";
          ...
          <div
            ...
            title={
              pending > 0
                ? undefined
                : signups.length === 0
                  ? "No signups to email yet"
                  : "Everyone on the waitlist has already been emailed"
            }
          >
            <button
              type="button"
              ...
              className="inline-flex items-center gap-1.5 h-8 px-3 rounded-full border border-[color:var(--apple-hairline)] bg-transparent text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              aria-describedby={pending === 0 ? "early-access-send-reason" : undefined}
            ...
            {pending === 0 && (
              <span id="early-access-send-reason" className="sr-only">
                {signups.length === 0
                  ? "No signups to email yet"
                  : "Everyone on the waitlist has already been emailed"}
              </span>
