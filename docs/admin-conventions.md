...
1. **Sell-panel Presses surface** — the partner sees only the inviting press's card (with a lock note reading "You were invited by {press name}.") until any of their albums has an order in `orders.fulfillment_status='shipped'`. Once a run ships, the full pressing-plant directory unlocks with the inviting press still floated to the front and highlighted. No "see other plants" disclosure or unlock-timeline language is shown to the partner while locked.
...
## Presses & operators can create an album — two doors, one approval boundary (Task #2044)

A press shouldn't have to wait for an operator to seed a release, but the platform also can't let a partner self-publish straight to fans. So album creation from the press surface has **two distinct doors** with a single trust boundary between them:

1. **Operator creates an album onto a press (no approval).** The press detail page (`/admin/manufacturers/:id`) Albums tab shows an **"Add album"** button **only for real operators** (`super_admin` / `admin` via `/api/me/role`) — partner admins viewing their own press never see it, and the server re-checks the role. It runs the same two-step New-Album flow as the global Albums index (`NewAlbumArtistDialog` mode `"album"` → `NewAlbumTitleDialog`), then POSTs the shared `POST /api/admin/albums` with an optional **`pressId`**. The server gates that field on operator role (403 otherwise), validates the press exists, creates the draft (prepping), and **homes it to the press immediately** via a zero-value `pressing_order_request` snapshot (`homeAlbumToPress`) — no approval, because an operator is trusted. Lands on the new draft's onboarding flow.

2. **A press invites an artist to start a draft (held for operator approval).** The Press Portal Invite dialog's **Artist** path is *not* the label `/invite` flow. The press searches **Spotify → Apple** (`/api/admin/spotify|apple/artist-search`, the same endpoints the operator's New-Album dialog uses) to prefill the artist's name + photo + streaming links — it **never browses the local People roster** (a press has no business seeing who else is on the platform). Picking a candidate + email + optional album title posts `POST /api/press/:id/start-album`, which reuses/mints the Person by email (stamping `invited_by_press_id`, but **not** `default_press_id` yet), creates an un-homed prepping draft, and mints an invite with `review_status='pending_review'`, `referrer_kind='manufacturer'`, `referrer_scope_id=<pressId>`, and `pre_flighted_album_id=<draftId>`. **No email is sent yet.** The button reads **"Send for approval."**

   The hold is a **creation-time** review (`review_status='pending_review'`), surfaced in the AdminInvites **"Held for review"** queue, which now shows the inviting press (`via <press>`) and the draft album title. **Approve** homes the draft to the press (`homeAlbumToPress`), pins the Person's `default_press_id` (only `WHERE … IS NULL`, so an existing home wins), *then* fires the invite email. **Reject** soft-deletes the orphan draft. The press-marker is the pair `referrer_kind='manufacturer' AND pre_flighted_album_id`, so this never collides with the artist/non-profit referral invites that share `referrer_kind`.

When changing either door, keep the boundary intact: operator creation auto-homes with no approval; press creation always holds until an operator approves, and homing + the `default_press_id` pin happen **at approve**, never at press-creation time.
...
**SPA / minimal-meta shops get site-specific handlers (Gruhn pattern).** Some vintage shops never expose usable OG/JSON-LD on the page itself. `guitars.com` (Gruhn) is a client-rendered SPA whose product data lives on the Drupal JSON API at `api.guitars.com`. `retrofret.com` and `gryphonstrings.com` are Shopify stores whose product pages are minimal-meta (Gryphon emits no JSON-LD `Product` and no `og:description`), so the scraper hits Shopify's public `/products/<handle>.json` endpoint (`SHOPIFY_JSON_HOSTS` set) and maps `title → name`, `vendor → brand/maker`, `variants[0].price → price`, `body_html → description`, `images[0].src → photo`, a leading 18xx/19xx/20xx token in the title → `specs.Year`, and any `Label:Value` tags (skipping Shopify's `Level N:` taxonomy nav tags) → specs. The maker slot reuses the same brand-resolution chain as resellers (`BRAND_ALIASES` → known maker host → catalog name match → name-only slot). All these handlers **fail loud** like the rest of the scraper — a sold/removed listing returns 404, an unreachable API 502, no silent garbage import. When a shop leaves a placeholder in Shopify's `vendor` field (Retrofret's "Tremoloa Maker") it surfaces as a name-only maker the operator can fix, never a fabricated domain. *Vintage Guitar magazine (`vintageguitar.com`) was evaluated and skipped — it's a WordPress/WooCommerce magazine, not a per-instrument storefront, so there's nothing to import into Add-gear.*
...
Currently wired: Gear ↔ Vendor, Gear ↔ Person, Album → Customer (album Dashboard city drill-down, album Customers/comped rows, album Buyers list → the fan detail page returns to the album). The customer page also honors the generic `?from=partner&backHref=…&backName=…` pass-through, so the Orders list, the shared order-detail Sheet (Fan Orders queue), a playlist's owner link, and the dashboard's recent-customers rows each return the operator to their own origin; the artist buyers list uses `?from=person`. The **Person/Artist page** (`AdminPerson.tsx`) consumes the crumb (breadcrumb + not-found) and receives `?from=instrument` (Gear credit → Person), `?from=album` (album/track credit → Person), and `?from=partner` (Label/Manager roster → Person). The **Album page** (`AdminAlbum.tsx`) also consumes the crumb — layered over its own `backToAlbumsHref` (which still handles the Albums list-state restore + embedded partner-portal catalog): it receives `?from=person&personId=` (Person catalog row → Album), `?from=label&labelId=` (Label Releases list → Album), and `?from=partner` (Manager Releases list → Album, since Manager isn't a keyed ORIGINS entry). The album crumb is suppressed in `embedded` portal mode. Vendor ↔ Label uses the same primitive — wire it in when that cross-section tab ships.
...
Fan side: `/api/songs/:id` returns `writers: string[]` derived from `trackPublishingSplits` (names only, never % or PRO). Player renders `Written by …` as the final line under LyricFlow™ lyrics, inside the bottom scroll mask.
...
## Public release page — slug, admin staging-preview, "View as a fan"

A release gets a public **Preview & Purchase page** at `get.goodtunes.music/<share_slug>` once it has a `share_slug` (set on the album editor). The page is the same desktop album view fans see, locked for non-owners (30s previews + Buy, left-nav bounces to login). The single-segment `/<slug>` convention is what's live today; artist-namespaced `/<artist>/<release>` (e.g. `nightbirde/hope`) is a planned routing follow-up — see `docs/roadmap.md`.

Two operator affordances make iterating on that page possible:

1. **Staging-preview.** `GET /api/public/album-by-slug/:slug` resolves a slug for a **privileged viewer** even while the release is still hidden / prepping / pre-sunrise (mirrors the `includeHidden` path `/api/albums/:id` uses). Everyone else still 404s until the release is buy-eligible. OG/unfurl (`server/og.ts`) stays gated, so a crawler can't surface a staged release. **This lets us perfect the real page with real data before flipping it out of "prepping."** Important host gotcha: `get.goodtunes.music` is hard-mapped to the **customer** auth kind, so an admin-kind session is rejected there — `isAdminUser` alone always returns false on the host that matters. The gate therefore *also* accepts a signed-in **full-access customer** (Bill's fan account, by email — the shared allowlist in `shared/fullAccess.ts`). To stage on the get host, sign in there with that fan account.
