...
<<<<<<< HEAD
   - **CarPlay audio (`com.apple.developer.carplay-audio`): ✅ Granted 2026-07-07.** The entitlement is committed in `App.entitlements` + the `Info.plist` CarPlay scene manifest. Apple approved the managed capability on 2026-07-07 (email to admin@gogoods.io). **Action required:** in the Apple Developer portal → **Identifiers → `Io.GoGoods.music`**, the **CarPlay** capability now appears — tick it and Save (same as Associated Domains / Push). Then delete stale App Store profiles (step 4 below) so the regenerated profile picks up CarPlay, and run a fresh distribution build.
...
4. **Clear out stale profiles** in the Apple portal → **Profiles**: delete any App Store profiles for `Io.GoGoods.music`. The pipeline's `--create` mints a fresh one that references the managed certificate and the App ID's capabilities. **CarPlay was granted 2026-07-07 — do this now** so the new profile picks up the capability (a cached profile without it keeps failing signing).
