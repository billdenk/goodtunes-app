...
Player-side product rules: SuperCredits™, the in-app chat demo, LyricFlow™ lyrics, playlist covers, favorites, and the song-row download model.
...
- **Fan read.** A track's rigs ride in the album credits payload (`bySongId[songId].rigs`). On **both** the mobile credits bottom-sheet and the desktop credits card, the **"On this track"** group renders each performer as a tappable door: when a performer's instrument matches a rig's base instrument (`rig.instrument.id === performer.instrumentId`) the door is labelled **`Rig ›`** and opens the **RigDetailSheet** (two performers sharing that instrument both get it; a performer with no matching rig instead opens their Person profile when they have a rich one, else stays plain text). An orphan rig whose base matches no performer renders as a personless rig door at the end of the group; a `trackRig` whose `rig` is null is omitted. The RigDetailSheet shows an artist-photo hero (track / artist / rig name + tweak note), the base instrument as its own tappable gear card, the accessories (carousel, See-All grid at ≥5; an accessory that links to a catalog instrument is itself tappable), the other tracks on *this album* that use the same rig (matched on the shared `rig.id`, shown only at ≥2), and a **two-state** availability CTA — **"Available from {vendor}"** when the base instrument resolves to ≥1 vendor, otherwise **"Request this rig"** which posts `POST /api/rigs/:rigId/request-quote` (works logged-out). Album-level production credits thread into the same song view as a tappable **Production** group. The resolver that turns the raw payload into the fully-hydrated `RigDetailView` lives once in `client/src/lib/rigViewModel.ts` (`makeResolveRigView` / `buildInstrumentsById`) and is consumed by both surfaces, so mobile and desktop stay in lockstep. (A middle "partial" availability state was dropped on purpose — see `docs/roadmap.md`.)
...
## Synced lyrics — LyricFlow™ (line-level shipped today)
...
A freshly-uploaded master now gets LyricFlow™ automatically in the background — no operator click required. When a new song's master finishes Mux ingestion, an orchestrator runs the whole LyricFlow™ pass automatically: transcribe + time-align lyrics (ElevenLabs Scribe), find the chorus to set `previewStartMs`, and auto-detect the `instrumental` and `isExplicit` flags. It reuses the existing engines (the same alignment + refinement + hallucination filter as the manual sync, the deterministic `[Chorus]`-marker → AI-fallback chorus finder, and a lyric profanity scan) — no new ML.
...
- **Status surfaced in the CMS.** `auto_goodsync_status` (`pending` → `processing` → `done` | `instrumental` | `failed`) shows as a badge in the per-track LyricFlow™ panel.

**Manual "Re-run LyricFlow™."** The LyricFlow™ panel has a per-song re-run control (`POST /api/admin/songs/:id/rerun-goodsync`) that runs the same orchestrator synchronously in **`force` mode** — overwriting operator-set cues / preview / flags (typed Plain lyrics are kept). It's gated behind a confirm dialog. This is distinct from "Re-sync with audio," which only re-aligns the synced-lyric cues.
