...
### Invited-partner & operator portals are THEME-AWARE (light + dark, ruled Aug 16 2026)

> **This section supersedes the earlier "light-only" canon.** Every invited-partner / operator portal — **Press, Vendor (Maker/Reseller), Artist, Label, Non-Profit, Manager, Publisher** — plus their invite panels, is **theme-aware with both light and dark modes**. The light mode is the admin (Stripe-leaning slate) theme; the dark mode is the **charcoal admin dark — never navy** (`#00062B` navy belongs to the fan shell only, which is a separate world untouched by this ruling). Design handoffs (Ruby) default to dark in mocks and ship **both token sets** per the theming rules in `handoff/style-guide/apple-canon.md`.

- **Mechanism (target).** Portal surfaces use the **token-based theming** the canon defines for theme-aware screens — no component hardcodes surface, ink, or hairline values. The previous instruction to hardcode slate classes is retired; existing hardcoded-slate portal screens migrate onto tokens (sweep in progress — acceptance gate: screenshot in both themes at 1440/1024/768; off-theme surfaces or overflow fail).
- **Mechanism (current shell).** `OperatorShell` adds `body.gt-admin` (had-guarded add/remove lifecycle) for these portal paths, and `client/src/main.tsx` first-paints `gt-admin` for the exact portal/`/invite` paths so there's no wrong-theme flash before hydration. The `isInvitedPortal` predicate in `scripts/design-lint.ts` exempts them from the fan-only `fan-text-tone` rule (they use admin tokens, not `text-fan-*`). Dark mode rides the existing `gt-admin-dark` charcoal ladder (see Admin dark theme notes).
- **Light token vocabulary** (mirror the admin frame, invent no new colors): page `bg-slate-50`; cards `bg-white ring-1 ring-slate-200`; nested fills `bg-slate-50`/`bg-slate-100`; dividers `divide-slate-100`/`border-slate-100`; text scale `text-slate-900` (titles) → `slate-700` → `slate-600` → `slate-500` → `slate-400`; hovers `hover:bg-slate-50`/`slate-100`. In dark mode these map through the theme tokens to the charcoal ladder — new code should express them via tokens, not raw slate classes.
