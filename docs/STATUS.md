...
- CD and Cassette pills are visible but disabled, marked "Coming" — no content behind them yet.
...
- CD and cassette: nothing exists on the press side or in the builder — only the disabled "Coming" pills.
...
- CD and cassette catalogs — only the disabled format pills exist; the data model and UI behind them haven't been designed yet.
