...
2. The server serves `assetlinks.json` at `https://goodtunes.music/.well-known/assetlinks.json`. The committed file at `public/.well-known/assetlinks.json` carries a `REPLACE_WITH_UPLOAD_KEYSTORE_SHA256_FINGERPRINT` sentinel; the route substitutes the real fingerprint from the **`ANDROID_RELEASE_SHA256`** environment variable. Get the fingerprint from Play Console → your app → **Test and release → App integrity → App signing key certificate** (this is the key that signs what users actually download, since Play App Signing is on), or locally with `keytool -list -v -keystore <upload.keystore>`. Paste the SHA-256 line into the env var (colons in, no quotes). ✅ **`ANDROID_RELEASE_SHA256` is set** (global Replit secret), so the route serves valid JSON with the real fingerprint (no sentinel, 200) on every host that reaches the deployment.

### Deep-link prerequisites — current state (verified)

Both env vars are set and the routes serve correctly. Confirmed by fetching `/.well-known/apple-app-site-association` (200, `application/json`, real Team ID, no sentinel) and `/.well-known/assetlinks.json` (200, real SHA-256, no sentinel) from the dev server and the `GoodTunes.replit.app` deployment URL. Two operator/infra items remain before stores will verify:

1. **Republish to activate the secret in the live deployment.** The route reads the env var at request time, but the *running* production build predates the newly-added `ANDROID_RELEASE_SHA256` secret, so `https://my.goodtunes.music/.well-known/assetlinks.json` still returns the 503 sentinel-not-set message until the app is republished from the main project (a task agent cannot publish). The AASA file already serves because `APPLE_TEAM_ID` was set before the current build. After republishing, both files serve their real values on `my.goodtunes.music` and `admin.goodtunes.music`.
2. **Attach the bare `goodtunes.music` apex to the deployment.** The deployment's custom domains today are `my.goodtunes.music`, `admin.goodtunes.music`, and `GoodTunes.replit.app`. The apex `goodtunes.music` is **not** routed here — `https://goodtunes.music/.well-known/...` returns an edge "Invalid .well-known request" 404 instead of our route. Because the iOS entitlements and the Android manifest both list `applinks:goodtunes.music` (the apex), Apple/Google fetch the association from the apex host; until the apex is added as a verified custom domain on this deployment (Publishing → custom domains), deep links on `goodtunes.music` (as opposed to `my.goodtunes.music`) will not verify. `server/auth/host.ts` already exempts `/.well-known/*` from the apex→`my.` 301, so once the apex reaches the app the file serves directly.

> **Package-name check before the Android build verifies:** `assetlinks.json` declares `package_name: fm.goodtunes.player` (matching `capacitor.config.ts` and the manifest). Confirm the Android app you actually upload to Play uses that same applicationId — if the published listing's package differs, App Links will not verify regardless of the fingerprint.

After the republish + apex domain are in place, run Apple's `swcutil verify -d goodtunes.music` and Google's [Statement List Tester](https://developers.google.com/digital-asset-links/tools/generator) on a real install to confirm both stores see the association before submitting. (No signed build / device was available in this environment, so these two verifier runs are the operator's last pre-upload check.)
