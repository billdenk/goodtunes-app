...
Both env vars are set, the routes serve correctly, and the **live production deployment already serves the real values on the canonical hosts**. Re-confirmed by fetching from the live deployment: `https://my.goodtunes.music` and `https://admin.goodtunes.music` both return `/.well-known/apple-app-site-association` (200, `application/json`, real Team ID, no sentinel) and `/.well-known/assetlinks.json` (200, `application/json`, real SHA-256, no sentinel). So the earlier "republish to activate the `ANDROID_RELEASE_SHA256` secret" step is **done** — the running build serves real values.
