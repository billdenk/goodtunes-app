...
| Application id (Android) | `com.gogoods_mobile` | Existing Play listing (kept for installs + reviews); set in `android/app/build.gradle`. Gradle `namespace` stays `fm.goodtunes.player` (Java package, independent of applicationId) |
...
> **Package-name check before the Android build verifies:** `assetlinks.json` declares `package_name: com.gogoods_mobile` (matching `android/app/build.gradle` → `defaultConfig.applicationId` and the kept Play listing). Confirm the `.aab` you actually upload to Play uses that same applicationId — if the published listing's package differs, App Links will not verify regardless of the fingerprint.
