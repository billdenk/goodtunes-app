...
**OAuth must round-trip to the originating host.** OAuth state is carried as an HMAC-signed bag inside the OAuth `state` parameter (stateless, no session storage) — this is required because Apple's `response_mode=form_post` callback is a cross-site POST and the session cookie (SameSite=Lax) is not sent on it. `callbackOrigin` returns the *exact* customer-family host the fan started on (`store` vs. `my`) when building the provider `redirect_uri`, so the IdP redirect matches registration and the signed state encodes the originating host. **This means `https://store.goodtunes.music/api/auth/google/callback` and `…/apple/callback` must be registered as allowed redirect URIs with Google and Apple alongside the existing `my.goodtunes.music` ones** — without that IdP-console registration, Google/Apple sign-in from the store host will fail at the provider. Email-code sign-in and the `?buy=1` bounce-back stay same-origin and need no IdP change.
...
### Press white-label host family — makesvinyl.com / pressesvinyl.com (Task #3258)

`*.makesvinyl.com` and `*.pressesvinyl.com` (including the bare apexes) are **flexible hosts** — `kindFromRequest` / `detectAuthKind` treat them like a `*.replit.dev` preview: path-based kind with `hostKnown=false` (host membership detected via `shared/whitelabelHost.ts` — one shared module so client and server can't drift). This is deliberate: the branded landing/login/estimate surfaces are customer-flavored, but press invites mint **admin-kind partner accounts**, and the invited artist must be able to accept and use their portal session on the same branded host. Forcing customer-kind would make `getAuthFromRequest` reject the freshly-minted admin identity (`server/whitelabelInviteAccept.db.test.ts` locks this in). Additionally, any OAuth start carrying an `?invite=` token resolves to admin kind on non-admin hosts so the callback's invite-grant branch runs (this also fixed OAuth invite-accept from dev previews); in production the admin-kind `callbackOrigin` is the canonical admin host, whose redirect URIs are already registered — so **OAuth invite acceptance needs no per-subdomain IdP registration**. A press with a `white_label_slug` on its `manufacturers` row (assigned in the press White Label tab, `PUT /api/press/:id/branding`, format-validated + case-insensitively unique) gets `https://<slug>.makesvinyl.com` — `pressesvinyl.com` answers too, but every link **we** mint uses the primary `makesvinyl.com` apex (`whitelabelOriginForSlug`).

- **What lives there:** estimate share links (`/e/:token`) and press-referred invite links (`/invite/:token`) generate on the press's branded host **in production when the slug is set** (`whitelabelOriginForPress` in `server/pressPortal.ts`; falls back to the request host otherwise, and always in dev). The login page and the bare-root landing skin themselves from the press's white-label brand via the public `GET /api/whitelabel/branding` (resolves the request host's slug; no auth — the skin is needed pre-session; dev can pass `?slug=`). Unknown subdomains and the bare apexes render a deliberately **neutral** page (no GoodTunes branding, no press enumeration, never an error).
- **Redirects:** these hosts are exempt from the canonical-host 301 by construction (`canonicalHostRedirect` only canonicalizes the goodtunes.music family), and they never appear in fan share links (`get.goodtunes.music` slugs are separate).
- **Auth:** session cookies are host-scoped, so email/password, email-code, and invite-token accepts work on these hosts with **zero IdP setup**. Google/Apple OAuth requires `callbackOrigin` to round-trip to the exact branded host (it does), **and Bill must register `https://<slug>.makesvinyl.com/api/auth/google/callback` and `…/apple/callback` with Google and Apple for each assigned subdomain** — until then the login page's OAuth buttons fail at the provider, but the password/invite legs work.
- **DNS (manual, Bill / GoDaddy):** connect both apex domains to the production deployment via the Replit Deployments → Settings → "Link a domain" flow, which issues the exact A/TXT records to enter in GoDaddy. Add a **wildcard CNAME** (`*` → the apex, e.g. `*` → `makesvinyl.com`) on each domain. Not in DNS ≠ broken app: until DNS lands, everything keeps falling back to the goodtunes.music hosts. **Verified live Aug 21 2026 (Task #3279)** with one important correction: **Replit does not issue wildcard TLS certificates** — the wildcard CNAME makes any `<slug>.makesvinyl.com` *resolve*, but HTTPS only works for hostnames **individually linked** in the Replit Domains panel (each gets its own cert). So assigning a new press slug has a one-time platform step: link `<slug>.makesvinyl.com` (and optionally `<slug>.pressesvinyl.com`) in Replit → Deployments → Domains. Thanks to the wildcard CNAME this verifies in ~1 minute with **zero GoDaddy edits** (the wildcard answers the CNAME lookup, and the TXT lookup falls through to the apex `replit-verify` record). Certs take a couple of minutes to mint after verification (SSL handshake errors until then). Bare apexes serve 200 with the neutral page and never 301 to goodtunes.music (verified).
- **Tests:** `server/whitelabelHost.test.ts` (parsing, kind resolution, redirect pass-through, callback origin).
