...
- **DNS (manual, Bill / GoDaddy):** connect both apex domains to the production deployment via the Replit Deployments → Settings → "Link a domain" flow, which issues the exact A/TXT records to enter in GoDaddy. Because presses get arbitrary subdomains, also add a **wildcard CNAME** (`*` → the deployment's domain target) on each apex so any `<slug>.makesvinyl.com` resolves without per-press DNS work. Not in DNS ≠ broken app: until DNS lands, everything keeps falling back to the goodtunes.music hosts.
