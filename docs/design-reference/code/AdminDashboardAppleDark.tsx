// AdminDashboardAppleDark — a DARK-MODE re-skin of AdminDashboardApple.
// SAME product, SAME mock data, SAME modules and layout. This is purely a
// nighttime re-skin: deep charcoal surfaces, soft elevation, quiet hairlines,
// text that glows gently rather than blares — the mood of macOS System
// Settings or Apple Music at night.
//
// Canon honored here:
//   • Admin dark surfaces are CHARCOAL, never navy or blue-tinted.
//   • The one accent stays GoodTunes blue (#319ED8), used exactly where the
//     light version uses it.
//   • The GoodTunes logo asset is dark-on-transparent → rendered white in the
//     dark header via a CSS invert/brightness filter.
//   • Partner press logos stay on WHITE circles, object-contain, never
//     recolored or inverted.
//   • No emojis. Pill buttons stay rounded-full.
//   • Severity colors get translucent washes on charcoal with legible accents.
//   • Recharts axes / gridlines / tooltip restyled for dark.
//   • Sticky translucent header is a dark blur; the left rail is a slightly
//     different charcoal than the page so the layering reads.
//
// All app plumbing is stubbed exactly like the light skin (react-query →
...
// Charcoal night palette. Everything is neutral charcoal — no blue/navy tint.
// Surfaces layer by lightness so elevation reads without hard chrome.
const INK = '#f5f5f7'; // primary text — glows gently, never pure white
const SUBINK = '#98989d'; // calm secondary label
const FAINT = '#6e6e73'; // tertiary / timestamps / disabled ticks
const HAIRLINE = 'rgba(255,255,255,0.10)'; // whisper-quiet card border
...
function RangeSwitcher({
  ...
  return (
    <div
      ...
      style={{ backgroundColor: CARD_SOFT, gap: 2 }}
...
// ─── Severity tokens — restrained, dark-appropriate translucent washes ─
...
> = {
  // Accents brightened for legibility on charcoal; washes are translucent
  // tints that glow softly against the card surface.
  critical: { accent: '#ff6b8a', wash: 'rgba(255,107,138,0.14)', label: 'Needs action' },
  warning: { accent: '#e8b04b', wash: 'rgba(232,176,75,0.14)', label: 'In transit' },
  ready: { accent: '#4cc98a', wash: 'rgba(76,201,138,0.14)', label: 'Ready to run' },
};

// Positive / negative delta colors, tuned for dark backgrounds.
const POS = '#4cc98a';
const NEG = '#ff6b8a';
const WARN = '#e8b04b';
...
function WorkQueueCard({
  ...
  return (
    <div
      ...
      style={{ backgroundColor: CARD, border: `1px solid ${HAIRLINE}` }}
    ...
    return (
      ...
      >
        <span
          ...
          style={{ backgroundColor: SEVERITY.ready.wash }}
        >
          <CheckCircle2 className="w-7 h-7" style={{ color: POS }} />
        </span>
  ...
  return (
    <section data-testid="work-queue">
      ...
      <button
        ...
        className="w-full flex items-center justify-between gap-3 rounded-2xl px-5 py-3.5 text-left transition-colors"
        style={{ backgroundColor: CARD, border: `1px solid ${HAIRLINE}` }}
        onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = CARD_SOFT)}
      ...
      >
        ...
        <span className="flex items-center gap-2.5 flex-shrink-0">
          ...
          <ChevronDown
            ...
            style={{ color: FAINT, transform: sectionOpen ? 'none' : 'rotate(-90deg)' }}
          />
        </span>
      </button>
      ...
      {tiles.map((t) => {
        ...
        return (
          <Link
            ...
            className="group rounded-2xl p-6 flex flex-col transition-shadow hover:shadow-lg"
            style={{ backgroundColor: CARD, border: `1px solid ${HAIRLINE}` }}
          >
            ...
            {d && (
              <div className="mt-3 flex items-center gap-1.5 text-[13px]">
                <span
                  ...
                  style={{ color: d.positive ? POS : NEG }}
                >
                  {d.text}
                </span>
      ...
      <div className="flex-1 min-h-[280px] flex flex-col">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={merged} margin={{ top: 6, right: 8, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id="appleTrendFillDark" x1="0" y1="0" x2="0" y2="1">
...
> = {
  ...
  npo: { Icon: HeartHandshake, color: POS },
        ...
        {visible.length === 0 ? (
          ...
          visible.map((it, i) => (
            <li key={`${it.kind}-${i}`} data-testid={`activity-${it.kind}-${i}`}>
              <Link
                href={it.href}
                className="flex items-center gap-3 -mx-2 px-2 py-2 rounded-xl transition-colors hover:bg-white/5"
              >
                {it.avatar ? (
...
function RankedRow({
  ...
  return (
    <li data-testid={`rank-row-${row.id}`}>
      <Link
        href="/admin/reports"
        className="block -mx-3 px-3 py-3 rounded-xl transition-colors hover:bg-white/5"
      >
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="text-[14.5px] font-medium truncate" style={{ color: INK }}>
                  ...
                  <span className="ml-2 text-[11px] uppercase tracking-wide" style={{ color: FAINT }}>
                    {b.ownerKind}
                  </span>
                </div>
                <div className="text-[12.5px] mt-0.5" style={{ color: SUBINK }}>
                  ...
                  {b.units === 1 ? '' : 's'}
                  {reason ? <span style={{ color: WARN }}> · {reason}</span> : null}
                </div>
              </div>
              <div className="text-right flex-shrink-0">
                ...
                <span
                  ...
                  style={
                    isBlocked
                      ? { backgroundColor: SEVERITY.warning.wash, color: WARN }
                      : { backgroundColor: SEVERITY.ready.wash, color: POS }
                  }
                >
...
// ─── Stub shell (nav + logo copied; re-skinned charcoal-calm) ────────
  ...
  return (
    <a
      ...
      style={{
        fontWeight: active ? 600 : 500,
        color: active ? INK : SUBINK,
        // Active row = raised charcoal pill on the rail — same treatment as
        // the segmented controls in the dashboard. Leave background undefined
        // when inactive: an inline `transparent` would override the hover:bg-*
        // class (inline styles always win).
        backgroundColor: active ? PILL_ACTIVE : undefined,
        boxShadow: active
          ? '0 1px 2px rgba(0,0,0,0.4), 0 0 0 0.5px rgba(255,255,255,0.06)'
          : undefined,
      }}
    >
      {Icon ? (
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          className="w-9 h-9 rounded-full text-[13px] font-semibold flex items-center justify-center flex-shrink-0 focus:outline-none focus-visible:ring-2 transition-shadow"
          ...
          {ADMIN_USER_MENU.map((m) => {
            ...
            return (
              <button
                key={m.label}
                type="button"
                className="w-full flex items-center gap-2.5 px-4 h-9 text-[13.5px] transition-colors hover:bg-white/5"
                style={{ color: INK }}
...
function StubShell({ children }: { children: ReactNode }) {
  ...
  return (
    ...
    >
      {/* Sticky, translucent, blurred top bar — a DARK blur (inline styles;
          Tailwind backdrop utilities are unreliable in this sandbox). */}
      <header
      ...
      >
        <div className="flex items-center min-w-0">
          {/* Logo asset is dark-on-transparent → invert + brighten to render
              it white on the charcoal header. */}
          <img
            src={goodtunesLogo}
            alt="GoodTunes"
      ...
      <div className="flex-1 min-h-0 flex">
        {/* Charcoal rail — a touch lighter than the page canvas (like macOS
            System Settings at night), raised charcoal pill on the active
            item, no hard chrome. */}
        <aside
              ...
              entry.kind === 'item' ? (
                ...
                <div key={entry.label}>
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-2.5 h-9 rounded-lg text-[13.5px] font-medium transition-colors hover:bg-white/5"
                    style={{ color: SUBINK }}
                  >
                    {open[entry.label] ? (
...
export function AdminDashboardAppleDark() {
...
export default AdminDashboardAppleDark;
