// NpoFirstRun — the DAY-ONE experience for a brand-new GoodTunes NON-PROFIT
// (cause) partner: Endometriosis Foundation of America ("EndoFound"), just
// onboarded, with ZERO referred artists yet.
//
// This is the empty-state sibling of the live NPO portal: it reuses the same
// partner shell (full-width top bar with the EndoFound wordmark + Feedback /
// bell / Jeanne Rebillard avatar dropdown, the NPO left rail, the "POWERED BY"
// GoodTunes footer, and the bottom scroll-fade) and the same light slate
// design language, but every data surface is scoped to a partner with nothing
// in it yet. The goal is that the empty dashboard reads as READY, not failure:
//
//   • No queue, no CTA to act on — there's nothing yet.
//   • The KPI strip keeps its 5 NPO tiles (Orders / New fans / Dollars donated
//     / Pending payout / Paid out) but shows em-dashes with quiet "your first X
//     lands here" microcopy — never red zeros or -100% deltas.
//   • The main area is a "Getting started" checklist ordered by NPO priority
//     (invite an artist → share referral link → invite team → foundation live).
//   • Recent activity holds a single friendly "Endometriosis Foundation joined
//     GoodTunes" business event.
//   • "Your artists" and "Donations" keep their card shapes but empty.
//
// On top of the empty dashboard, a LIGHT welcome modal (Apple-Music-style
// dim-and-blur treatment, INLINE styles for the backdrop) opens on first load.
//
...
// ─── Empty KPI strip ─────────────────────────────────────────────────
// Modeled on the real NPO dashboard KPIs, but each value is an em-dash with a
// quiet "your first X lands here" line. Never red zeros or -100% deltas — an
// empty cause-partner account should read as ready, not failed.
...
const EMPTY_KPIS: EmptyKpiTile[] = [
  ...
  { id: 'newFans', label: 'New fans', hint: 'Grows as your artists find fans' },
  { id: 'donated', label: 'Dollars donated', hint: 'Your first donation lands here' },
...
// ─── Getting-started checklist (replaces the main data area) ─────────
// Ordered by NPO priority. No numbers.
...
const CHECKLIST: ChecklistStep[] = [
  ...
  },
  {
    ...
    detail: 'Send one link and let artists join your cause without individual invites.',
    done: false,
  },
  {
    id: 'team',
    ...
    >
      ...
      <div className="flex-1 min-h-0 flex flex-col items-center justify-center text-center py-8">
        <span className="w-14 h-14 rounded-full inline-flex items-center justify-center" style={{ backgroundColor: '#f2f2f5' }}>
          <Users className="w-6 h-6" style={{ color: SUBINK }} />
        </span>
        <p className="mt-4 text-[15px] font-semibold" style={{ color: INK }}>
          Artists you refer will show here
        </p>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-h-0 flex flex-col items-center justify-center text-center py-8">
        ...
        <p className="mt-1.5 text-[13px] max-w-xs leading-relaxed" style={{ color: SUBINK }}>
          As your artists sell, each donation will post here — line by line.
        </p>
      </div>
    </div>
  );
...
function NpoShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="h-screen flex flex-col font-sans" style={{ backgroundColor: CANVAS, color: INK }}>
      {/* Full-width top bar — EndoFound wordmark left; Feedback, bell, Jeanne
          avatar right. The wordmark is the partner brand; never truncates. */}
      <header
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* GoodTunes logo, LARGE, with generous space below (inline styles). */}
        <img
          src={goodtunesLogo}
          alt="GoodTunes"
        ...
        <p className="mt-3 text-[14px] leading-relaxed" style={{ color: SUBINK }}>This is your foundation's home base. Every dollar raised by the artists you refer (or those who we send directly to you) shows up right here, all in one place.</p>
...
export function NpoFirstRun({ showWelcome = true }: { showWelcome?: boolean } = {}) {
    ...
    <>
      <NpoShell>
        <div className="flex flex-col gap-5">
          ...
          <div className="flex items-end justify-between gap-3 flex-wrap">
            <div className="min-w-0">
              ...
              <p className="text-[14px] mt-1" style={{ color: SUBINK }}>
                Your home base is ready — donations from the artists you refer
                will land here.
              </p>
            </div>
          ...
          {/* Getting started replaces the main data area; activity recedes */}
          ...
          {/* Bottom row — empty Your artists + Donations ledger */}
