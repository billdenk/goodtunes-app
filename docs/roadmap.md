...
- **Tier 3:** Native iOS/Android wrapper for cert pinning, encrypted local cache, jailbreak detection. Defer until paying users + native app on roadmap.
  - **Encrypted local cache — shipped.** Native offline downloads now encrypt audio at rest with AES-256-GCM, stored in private, backup-excluded app storage and revoked when entitlement lapses (`client/src/lib/nativeDownloads.ts`).
  - **Hardware-backed device key + jailbreak/root gate — shipped.** The per-device master key now lives in the platform hardware secret store — iOS Keychain (`kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`) / Android Keystore (TEE/StrongBox-sealed data key) — via the in-tree `SecureKeyStore` Capacitor plugin (`ios/App/App/SecureKeyStorePlugin.swift`, `android/.../SecureKeyStorePlugin.java`, JS wrapper `client/src/lib/nativeSecureKey.ts`). `getDeviceKey` reads from hardware and falls back to the legacy sandboxed-IndexedDB `CryptoKey` if the plugin is unavailable; existing downloads re-encrypt onto the hardware key once (`migrateToHardwareKey`). A best-effort jailbreak/root probe gates the download path — on a compromised device new downloads are refused and existing files won't decrypt (playback falls back to online streaming); the probe fails safe so a false alarm never blocks a legitimate user.
  - **Certificate pinning for download fetches — shipped.** The offline-download fetch is routed off the WebView (which doesn't honor pin config) through a native TLS-pinned HTTP path (`SecureKeyStore.pinnedDownload` → URLSession / HttpsURLConnection), and GoodTunes' own hosts are pinned to the long-lived ISRG (Let's Encrypt) **roots** — X2 primary, X1 + YE backups — so a MITM on a GoodTunes host can't swap a master while routine ~60-day leaf renewals never brick installs. Declared in `ios/App/App/Info.plist` (`NSPinnedDomains`) and `android/.../res/xml/network_security_config.xml` (with an anti-brick `expiration`). Scoped to `goodtunes.music` only, so legacy Dropbox masters (different CA) are unaffected until migrated to object storage; older builds fall back to the WebView fetch but a pin failure never silently downgrades. Full rationale, the exact pins, and the safe rotation runbook live in [`docs/cert-pinning.md`](./cert-pinning.md).
...
## CarPlay cold-connect tap-to-play (implemented — pending device verification)

**Status: implemented in-repo (2026-07-14) — awaiting a manual `ios-testflight` build + on-device CarPlay pass before it counts as shipped.** The cold-connect *browse* case has been done for a while: connecting to a head unit with the phone app fully quit renders the last-known Home / Collection / Recents and the real last Now Playing track straight from an on-device snapshot (`NowPlayingStore` disk persistence, hydrated on `didConnect`), with no phone interaction and no network. **Starting audio from a true cold connect** — tapping Play / an album / a track when the app was never opened — is now implemented too.

Why it was hard: all audio is produced by the web player's hidden `<audio>` element inside the Capacitor WebView. On a cold car connect iOS spins up *only* the CarPlay scene — the phone window scene that hosts the web player is never created, so there is no `<audio>`, no signed Mux session, nothing to drive.

How it works (manifest-free — the `UIApplicationSceneManifest` is untouched; two prior manifest changes black-screened the store-signed binary, see `.agents/memory/ios-scene-manifest-black-screen.md`):

1. **Headless bring-up** — `HeadlessWebPlayer` (in `ios/App/App/MainViewController.swift`) instantiates the storyboard's `MainViewController` off-screen when the CarPlay scene connects and no bridge VC is live (`CarPlaySceneDelegate.didConnect` → `bringUpIfNeeded()`). The web player boots, resumes the signed-in session, and registers the normal plugin callbacks. If the phone app opens later, `SceneDelegate` **adopts** the same VC as its window's root — never a second web player.
2. **Pending-tap buffering** — a browse tap landing before the plugin loads is buffered in `NowPlayingStore.pendingIntent` (last tap wins) and drained by `NowPlayingPlugin.load()`; the play-intent emits (`playAlbum`/`playIndex`/`playPlaylist`) use `notifyListeners(retainUntilConsumed: true)` + a `ts` payload so they survive the remaining gap until JS attaches its listener, and JS drops anything older than 2 minutes (a retained tap from a boot that never finished must not blast audio hours later). `PlayerContext` additionally stashes a `playAlbum` command that arrives before the library queries resolve and replays it when they land.
3. **Audio-session etiquette** — a headless boot defers `AVAudioSession.setActive(true)` until the first genuine play intent (one-shot), so merely connecting the car doesn't interrupt whatever the fan was already listening to (Spotify, radio). The normal phone launch keeps activate-on-load.
4. **Kill switch** — `configureHeadlessBringUp()` in `nativeNowPlaying.ts` persists a `HEADLESS_BRINGUP_ENABLED` constant into native UserDefaults on every boot; flipping it to `false` + web-publishing disables the bring-up in the field without a native rebuild.

Verification is device-only (no Xcode / head unit in the container), so this ships behind the existing `CARPLAY_GRANTED` gate alongside the rest of CarPlay. The Swift half needs a manual `ios-testflight` Codemagic build; see the CarPlay checklist in `docs/native-builds.md` for the on-device pass (cold-connect tap → audio within ~15s, no-tap connect leaves other apps' audio untouched, phone-open-later adopts the same player). Until that pass is recorded, the honest boundary for anything customer-facing stays: **cold connect = browse + metadata; tap-to-play is implemented but unverified on hardware** — `docs/capabilities.md` must not claim it yet.
