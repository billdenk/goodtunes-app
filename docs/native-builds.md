...
> **Status (2026-07-04): iOS CarPlay code-complete + reviewed in-repo; on-hardware pass still outstanding.** Android Auto has been removed (Play Console rejection). CarPlay wiring is committed and reviewed end-to-end — the iOS scene/entitlement/plugin and JS bridge line up and drive playback bidirectionally. What remains is the on-device CarPlay pass, which **cannot run in the Replit container or in CI** (no Xcode, no head unit), so it is deferred to an operator with a CarPlay unit or the Xcode CarPlay simulator. **When that pass is run, record its dated result here** — device + head-unit (or simulator) versions and pass/fail per checklist item.
