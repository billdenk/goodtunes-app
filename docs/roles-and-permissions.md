...
- **Ambassador invites.** An NPO with `invite_subusers` can additionally promote one of its contact Persons to "ambassador" — that Person row's `can_invite_ambassadors` flag (super-admin-flipped via `PUT /api/admin/people/:id/ambassador-toggle`) lets the ambassador mint artist invites that referrer-chain back to the NPO via `people.referred_by_org_id`.
