...
965 of the export's profile-only users are status `PENDING`. The
operator-confirmed case is absent from the Stripe export CSVs
(`unified_payments-*`, `unified_customers-*`) too.
...
export only ever carried profiles for the rest. Operator-confirmed for one
case: Bill confirmed that fan is absent from the Stripe DB and supplied their
Jan 2026 goGoods support thread — they were *locked out trying to create an
account* (never got their code), so they never completed sign-up or reached
...
these fans' rows — verify by spot-checking that a known profile-only fan has ≥1
