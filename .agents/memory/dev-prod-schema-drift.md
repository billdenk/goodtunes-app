...
## Aug 21 2026 — three DROP-warning incidents in one day
In-flight task merges repeatedly landed new columns on PROD but not dev (the task envs' run_sql_both ran against prod while dev was skipped), so every republish attempt showed data-destroying DROP warnings (redemption_email_*, brand_*, email_branding, songs.source_url, estimates_white_label_enabled). Standing response: read the column's type from prod information_schema, ADD COLUMN IF NOT EXISTS on dev with the exact type/default, append an idempotent run_sql_both guard in scripts/post-merge.sh, commit+mirror, then have the user cancel and Republish. Never approve a publish showing a DROP on a populated column.
