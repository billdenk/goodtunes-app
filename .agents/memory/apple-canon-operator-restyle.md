...
- Global sweep (Aug 2026, Bill's "apply globally" handoff): canon now lives in the SHARED header primitives — AdminPageHeader (30px semibold two-tone h1, hairline via --apple-hairline), AddEntityButton (the one filled blue pill), ViewModeToggle (pill segmented track), AdminFilterPanel (round ghost trigger + pill chips) — so ~30 admin/partner index pages inherit it. Restyle those primitives, not per-page copies; detail-page h1s use the same 30px/semibold/-0.02em class. New canon text-[13px]/text-[30px] sizes are baselined in design-lint (5690).
...
**Page headers & range toolbar (ratified 2026-08-18, in docs/apple-canon.md):** no boxed page-header strips — the in-content two-tone heading IS the page title (breadcrumb → heading → intro line); the date-range toolbar lives only on metrics screens (Dashboard/Reports) — "the range picker travels with charts, never with the shell." When removing a shell picker, check every pane that consumed its qs: panes with no visible control must either pin an explicit all-time window (Orders) or grow their own in-content switcher (Reports hub rangeControl prop), or they silently stay filtered to a hidden 30-day default.
