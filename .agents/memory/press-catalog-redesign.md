...
**Portal chrome (Bill, Aug 2026):** OperatorShell leftnav top bar is FULL-WIDTH above the rail (brand at far window-left, never right of the sidebar search); press nav icons follow the playground mock (Clients=Users, Acquisition=UserPlus, Catalog=Library, Referrals=Gift). Catalog page now has explicit reorder MODE (never draggable at rest; Done→existing reorder endpoints, types = per-tier position PATCHes until a batch route ships), the "Colors in your catalog" search popover, and the "Edit type." editor whose size chips are display-only + Archive disabled (no persistence routes yet — see follow-up tasks).

## Dark mode (item 25, 2026-08-09)
- Dark reference is handoff/press-catalog/PressPackagePricingTableRunsDark.tsx; docs/apple-canon.md is the standing style guide (merged handoff/style-guide version + the repo's 2026-08-08 ratified amendments — keep both when it moves again).
- Mechanism: CSS vars flip most surfaces; inline light literals branch on `useAdminDark()` (new hook in client/src/lib/adminAppearance.ts, useSyncExternalStore on the appearance listeners). frostedPanel(dark)/fieldInput(dark) factories; dark constants (SEL_WASH/CRITICAL_WASH/CARD_SOFT/PILL_ACTIVE/DISC_RIM) char-identical to the ref.
- VinylDisc/ColorBall (PressVinylColors.tsx) carry the dark disc rim via useAdminDark — this follows the PAINTED theme, which is canon ("vinyl art on dark surfaces gets the rim"); gt-admin-dark only ever paints on admin/portal hosts (main.tsx isAdmin branch), fan surfaces never get it. An architect review flagged this as a leak — it is not; don't "fix" it by prop-drilling unless Bill complains.
