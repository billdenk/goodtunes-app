...
- Save sends `pressTierId` + `pressColorId` to the SKU upsert; the server resolves names and snapshots them onto the SKU as `vinylColorTier` (tier name), `vinylColor` (color display name), `quantityTier` (snapped qty), `costSource: "catalog"`. As of the color-save reliability fix the upsert ALSO pins the exact identity — `album_skus.press_id` / `press_tier_id` / `press_color_id` (nullable; null on the placeholder/legacy path). The SellPanel now re-resolves a saved color id-FIRST (then name, then UNRESOLVED — never silent first-color fallback), so renaming/re-importing a tier no longer silently re-homes a saved SKU. Checkout/cart/payout still read the name snapshots; the ids are for stable re-resolution in the editor.
...
- `press_colors` reference the tier by `tier_id`, so **renaming a tier keeps its swatches**. The SellPanel re-picks a saved SKU's tier/color id-first now (`album_skus.press_tier_id`/`press_color_id`), falling back to name then to an explicit UNRESOLVED state — so renaming a tier no longer silently re-homes a saved SKU onto Black; a re-import that regenerates ids surfaces "Previously X — choose a color" instead. Still prefer renaming only unpriced/unused tiers to avoid forcing operators to re-pick.
