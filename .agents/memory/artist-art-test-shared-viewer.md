...
- The ART the artist viewer seats comes ONLY from the authed completed-template art-file endpoint (full artboard, pdf.js at real mm). Never fall back to `component.previewUrl` as viewer art — that raster is the TRIM/front-panel crop, and seating it looks like "only the cover printed, back panel empty". If no full-bleed raster is obtainable (dead legacy external link, render failure) show an explicit "preview unavailable" state. The art-file route also self-heals pre-mirroring rows by mirroring the external link on first read.
