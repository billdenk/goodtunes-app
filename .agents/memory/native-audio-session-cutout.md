...
**Root cause (high-confidence, from architecture analysis):** the in-tree native
`NowPlayingPlugin.swift` calls `configureAudioSession()` — which does
...
**The eventual native fix (needs a Codemagic rebuild):** remove
`configureAudioSession()` from the recurring `setMetadata`/`setPlaybackState`
paths; call it once on `load()` and on the transition to playing; add
`AVAudioSession` interruption + routeChange observers so the app cooperates with
(instead of fighting) the WebView session.
