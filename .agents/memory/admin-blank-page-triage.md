...
- **Stale/orphaned bundle after redeploy.** A page holding an old `index.html` whose content-hashed `<script>`/`<link>` 404s → React never mounts → silent white. There is now an automatic self-heal (`client/src/lib/bootHeal.ts` + capture-phase asset-error listener in `GlobalErrorBoundary.tsx` + watchdog in `main.tsx` + `markBootSucceeded` in `App.tsx`; see `docs/auth-and-dual-shell.md` → "Self-heal on boot failure"). Two durable lessons worth keeping: (1) resource-load (`<script>`/`<link>`) failures do **not** bubble, so you must listen in the **capture** phase or they're invisible — this is why the screen went white with nothing in the window `error` handler. (2) Any auto-reload self-heal **must** fail closed: only reload if a cross-reload guard actually persisted (sessionStorage stamp **or** `window.name` fallback), else paint the banner — a guard that silently no-ops when storage is blocked (private mode) turns into an infinite reload loop. Net effect for triage: a *fresh* white screen now self-recovers once; a *permanent* banner means a genuinely broken deploy (bad bundle hash), not stale client state.
