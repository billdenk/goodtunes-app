...
- Press-portal edit gates now resolve the caller from the bearer stamp first (`adminUserId ?? session userId`) instead of relying on the backfill side effect — prefer that explicit pattern over raw session reads when adding new caller-identity checks behind bearer-capable gates.
