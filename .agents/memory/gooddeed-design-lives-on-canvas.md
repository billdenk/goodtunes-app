...
- **Square share card background — Bill locked TWO approved treatments** (both full-bleed SHARP album art, NOT the old blurred-everything look, which he rejected because "people don't really see the album"):
  ...
  - These are exposed as knobs on the mockup (`bg=bleed-dark`, optional `gblur=NN`); the mockup default render is still the old slab (untouched).
  - **D is now SHIPPED.** The fan-facing card (`client/src/components/GoodDeedCertificate.tsx` `CertCard`) renders the "D" full-bleed treatment for all three shapes: a single `absolute inset-0 object-cover object-top` cover + the `BLEED_SCRIM` constant (`linear-gradient(180deg, rgba(0,6,43,0.18) 0%, …0.40 40%, …0.86 72%, …1 100%)`), with a transparent top spacer (`artBandU`) holding the avatar/text rhythm. The old per-shape `grad` slab field was removed. **E (`gblur`) was NOT enabled** — Bill approved D only.
