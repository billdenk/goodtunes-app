...
## Bounded retry ladder (sourced rows that genuinely error)
`album_videos` now carries `mux_retry_count` / `mux_last_retry_at`, mirroring songs. A bonus video that DOES have a real `/objects/` source but whose Mux conversion genuinely errors used to be re-attempted by `reconcileMuxVideos` on every interval forever (the old code filtered `muxStatus === "errored"` into the ingest list unconditionally). It now rides the SAME `BACKFILL_MAX_ATTEMPTS` / `nextBackfillDelayMs` ladder as songs via a shared `videoRetryGate(v, isBoot)` closure: errored rows back off exponentially and stop after the cap (leaving the row terminally `errored` → admin warning badge + fan "unavailable"); never-ingested rows (no asset, status null) still retry every sweep. The attempt is STAMPED by the caller (reconcile loop AND the playback-url lazy-ingest path) before kicking `maybeIngestVideoToMux`, never inside the helper — same split as the song backfill. Ladder resets to 0/null on success (webhook `ready`, reconcile heal→ready, ingest-create→ready) and on a videoUrl change in `PUT /api/admin/album-videos/:id`. No manual per-video retry route exists (unlike songs' `/mux-ingest`), so an operator overrides the cap by re-saving the row with a new file.

**Why:** sourceless rows (empty `video_url`) were already handled (Task #1459 soft-deleted them); this closes the OTHER half — a sourced row with a permanently-broken master quietly spamming Mux with no "gave up" state.
