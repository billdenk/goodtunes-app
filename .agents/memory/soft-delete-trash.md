...
- **Natural-key unique indexes on a soft-deletable table MUST add `AND deleted_at IS NULL` to their WHERE predicate**, or a trashed row permanently squats its slug/domain and re-creation 500s on a raw 23505. Covered so far: `vendors_domain_top_uniq`, `labels_domain_unique`, `manufacturers_domain_unique`, `fulfillment_partners_domain_unique`, `albums_share_slug_unique`. The matching partial index is hand-applied (drizzle-kit won't push WHERE clauses): canonical copy in `scripts/post-merge.sh` + a one-off in `scripts/prod-schema-fixups/`. Also drop any `includeTrashed` in the route's pre-create natural-key lookup (the album share-slug PUT had one) so the app layer doesn't re-introduce the squat. People have NO share slug despite task lore; only `legacy_gogoods_id` (import-dedup, out of scope).
