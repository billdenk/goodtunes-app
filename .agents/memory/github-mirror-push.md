...
## Task-agent heal for DIVERGED blocking markTaskComplete
...
**RESOLVED 2026-08-10:** Bill added a NEW ed25519 deploy key (old June key deleted on GitHub, private half lost in the migration); `GITHUB_MIRROR_DEPLOY_KEY` secret restored. Main-agent shell DOES see the secret now, and a direct `git push --force <ssh-url> HEAD:refs/heads/main` from the main-agent shell WORKED — the "force blocked for main agent" claim below is stale. "DIVERGED" from the freshness check can just mean Ruby's design-handoff commits sit on GitHub main; project main is authoritative and the force-push over them is expected.

**Secret DROPPED from the whole environment (observed 2026-08-10, resolved same day):** `GITHUB_MIRROR_DEPLOY_KEY` no longer appears in the repl's secret list at all (likely the same Enterprise migration that dropped the prod Stripe connector) — not just hidden from the main-agent shell. Consequence: `mirror-freshness` SKIPs and every merge's `sync_github_build_mirror` silently self-skips, so the mirror goes stale until Bill re-adds the private deploy key as the secret or pushes manually from the Git panel. Republish never touches GitHub.
...
## Mirror push gotchas (2026-08-12)
- A mirror-only `chore(mirror): repoint npm firewall lockfile URLs…` commit sitting ON TOP of project main also reads as DIVERGED to the freshness check. Heal = force-push the project main tip (main-repl/main) over it from a task agent; the chore commit is doomed on the next merge's force-push anyway. Note the local `refs/remotes/ghmirror/main` fetch can fail on a stale `.lock` — rm it, then `git update-ref`.
- `ssh-keygen -y -f` on the rebuilt deploy key can fail with "error in libcrypto" even when the key WORKS — skip that pre-check; validate with `git ls-remote` instead. Use the exact `write_normalized_deploy_key` recipe from scripts/post-merge.sh.
- Ruby (design studio) now pushes handoff commits directly to the mirror's main. A rejected push = fetch, confirm her commits touch ONLY `handoff/`, then merge FETCH_HEAD (precedent: merge commits authored as GoodTunes) and push. This supersedes the old "never pull/merge github/main" rule for handoff-only commits; still never merge non-handoff drift blind.
