...
- Second failure mode (not a crash, just invisible UI): when a `backdrop-filter` element renders *above* another fixed `backdrop-filter` element, iOS WebKit drops the upper element's background fill — border + text only, no surface. Symptom: toast over the PlaylistPickerSheet scrim looked like a pitch-dark capsule. Fix is the same direction — for small always-on chrome (toasts, pills) prefer fully opaque bg + shadow over backdrop-blur. Reserve blur for ONE surface at a time.
